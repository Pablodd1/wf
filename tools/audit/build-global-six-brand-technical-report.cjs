...
function build(findings, completion) {
  ...
  const sources = [{
    ...
    query: {
      engine: 'DuckDB',
      language: 'sql',
      sql: "SELECT b.brand, b.catalog_references, b.trading_floor_listings, b.wts, b.wtb, b.price_research_qualified_wts, b.analytics_ready_references, b.decision FROM read_json_auto('audit-output/global-six-brand-completion/brand-findings.json') f, UNNEST(f.brands) AS t(b)",
      description: 'Cursor-bounded Trading Floor inventory and adaptive four-reference Price Research summaries, with row-level raw messages removed before checkpointing.',
  ...
  return {
    ...
    manifest: {
      ...
      blocks: [
        ...
        { id: 'definitions', type: 'markdown', sourceId: 'global_contract', body: '## One contract now governs identity, price, dealer, counts, and publication\n\nA reference must be an exact brand-scoped canonical identity. Price Research accepts current single-watch WTS only when source amount and currency are explicit, dated independent FX is available when needed, and duplicate/bundle/publication gates pass. Total Listings remains distinct published WTS plus WTB and never depends on price verification. Dealer display uses the strongest evidence-backed dealer, seller, or poster identity; generic placeholders never become identities or ratings.' },
        ...
        { id: 'limitations', type: 'markdown', body: '## Coverage is partial where production access or parity failed\n\nRolex and Patek population totals are not inferred from capped customer API pages. Cartier qualified-WTS counts remain unknown because two customer-facing sources disagree. The long-running global reference census is resumable but not labeled complete. These limitations prevent deployment; they do not downgrade previously confirmed defects.' },
