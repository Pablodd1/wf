...
for(const [container,database] of [['supabase_db_wf-final-disposable','postgres'],['wf-final-disposable-pg18','wf_production_forward_20260907']]){
 ...
 const indexed=fs.readFileSync(path.resolve(__dirname,'../../supabase/migrations/20260909190000_indexed_materialization_cursor.sql'),'utf8').replace(/\bBEGIN;/,'').replace(/COMMIT;\s*$/,'');await db.query(indexed);await db.query("SET LOCAL plan_cache_mode='force_generic_plan'");
 const scope='SYNTHETIC-INDEXED-MATERIALIZATION-'+crypto.randomUUID(),ids=[],texts=[];
 ...
 const firstWindow=(await db.query('SELECT raw_row_id FROM wf_canonical_staging.read_materialization_member_window_v2($1,NULL,500)',[scope])).rows.map(r=>r.raw_row_id);assert.deepEqual(firstWindow,[...ids].sort());
 const afterWindow=(await db.query('SELECT raw_row_id FROM wf_canonical_staging.read_materialization_member_window_v2($1,$2,500)',[scope,firstWindow[0]])).rows.map(r=>r.raw_row_id);assert.deepEqual(afterWindow,firstWindow.slice(1));
 for(const role of ['anon','authenticated','service_role'])assert.equal((await db.query("SELECT has_function_privilege($1,'wf_canonical_staging.read_materialization_member_window_v2(text,uuid,integer)','EXECUTE') allowed",[role])).rows[0].allowed,false);
...
main().catch(e=>{console.error('INDEXED_MATERIALIZATION_TEST_FAILED',e.code||e.name,e.message?.slice(0,200));process.exitCode=1;});
