...
test('non-watch release is pinned, allowlisted, audited, and bounded', () => {
  ...
  assert.match(workflow, /FROM public\.raw_message_versions rv/);
  assert.match(workflow, /bounded_exists_per_category/);
  assert.doesNotMatch(workflow, /count\(\*\) FILTER \(WHERE category=/);
