...
def build_patek_reconciliation(existing_site_hashes, existing_site_ids, downloads_dir, desktop_dir):
    canonical_brand = "Patek Philippe"
    db_aliases = ["Patek Philippe", "Patek", "Patek Phillipe", "CUBITUS", "PP"]
    output_filename = f"Patek_Philippe_Codex_Reconciliation_Master_{datetime.now().strftime('%Y-%m-%d')}.xlsx"
    ...
    for record in rows:
        ...
        sheet1_corrections.append([
            clean_xml(rec_id), clean_xml(rec_id), clean_xml(unique_key), clean_xml(payload_sha256),
            clean_xml(canonical_brand), clean_xml(model_val), clean_xml(ref_val), clean_xml(dial_col),
            clean_xml(cond_val), clean_xml(l_type), clean_xml(raw_msg), clean_xml(raw_price_str),
            clean_xml(curr), norm_price_usd, clean_xml(p_ev_type), clean_xml(post_date),
            clean_xml(seller_src_id), clean_xml(img_url), clean_xml(corr_action), clean_xml(corr_reason),
            clean_xml(rev_status)
        ])

        sheet2_price_evidence.append([
            clean_xml(rec_id), clean_xml(payload_sha256), clean_xml(raw_price_str), clean_xml(raw_price_str),
            clean_xml(curr), norm_price_usd, clean_xml(fx_src), fx_rate,
            clean_xml(post_date[:10] if post_date else now_str[:10]), clean_xml(p_ev_type),
            clean_xml(p_research_elig), clean_xml(excl_reason)
        ])

        sheet3_dealer_linkage.append([
            clean_xml(rec_id), clean_xml(payload_sha256), clean_xml(seller_src_id), "WhatsApp",
            clean_xml(record.get('region') or "GLOBAL"), clean_xml(unique_key), clean_xml(seller_src_id),
            "EXACT_SOURCE_PROFILE_ID", "VERIFIED" if record.get('from_name') else "UNLINKED"
        ])

        img_assoc = "EXACT_LISTING_IMAGE" if img_url else "NO_SOURCE_IMAGE"
        sheet4_images.append([
            clean_xml(rec_id), clean_xml(unique_key),
            clean_xml(hashlib.md5(img_url.encode('utf-8')).hexdigest()[:12] if img_url else "NO_IMG"),
            clean_xml(img_url), 1, "PRIMARY_FRONT_IMAGE", clean_xml(img_assoc)
        ])
    ...
    print(f"  -> Saving Patek Philippe master workbook to {down_path}...", flush=True)
    wb.save(down_path)
    shutil.copy2(down_path, desk_path)
    print(f"  -> Successfully generated & copied Patek Philippe workbook to Desktop & Downloads!", flush=True)
        ...
        try:
            ...
            for item in data:
                if isinstance(item, list) and len(item) > 0:
                    existing_site_ids.add(str(item[0]))
                    if len(item) > 8 and item[8]:
                        existing_site_hashes.add(hashlib.sha256(str(item[8]).encode('utf-8')).hexdigest())
        except Exception as e:
            print(f"Error loading parsedWatches.json: {e}", flush=True)

    build_patek_reconciliation(existing_site_hashes, existing_site_ids, downloads_dir, desktop_dir)
