'use strict';
// Local only. Read completed immutable V1 candidate parts once, then read only
// affected original source rows. Neither V1 outputs nor raw source are edited.
...
function findAnchorIssues(candidate) {
  const view = v1.parsingView(candidate.source_context_text), issues = [];
  for(const anchor of nonReferenceAnchors(view.text)) {
    const start = view.starts[anchor.start], end = view.ends[anchor.end - 1];
    for(const block of candidate.source_spans) {
      assert.equal(block.quote, candidate.source_context_text);
      const a = block.start + start, b = block.start + end;
      if(!(candidate.evidence.reference || []).some(r => r.field === block.field && r.start < b && r.end > a)) continue;
      const quote = view.points.slice(start, end).join('');
      issues.push({ reason: anchor.reason, field: block.field, field_sha256: block.field_sha256,
        start: a, end: b, offset_unit: 'UNICODE_CODEPOINT', end_exclusive: true, quote, quote_sha256: sha(quote) });
    }
  }
  return issues;
}
...
async function run(config) {
  ...
  const files = ['expanded-evidence-candidates.cjs', 'expanded-evidence-candidates-v2.cjs', 'expanded-reference-anchor-policy-v2.cjs', path.basename(__filename)];
  ...
  assert.equal(base.binding.parser_sha256, v2.DEPENDENCY_HASHES.base_parser);
  ...
  const state = fs.existsSync(checkpointFile) ? read(checkpointFile) : { contract: 'WF_EXPANDED_REFERENCE_ANCHOR_CORRECTIONS_V2', status: 'RUNNING_LOCAL_ONLY', started_at: new Date().toISOString(), binding, processed_parts: [], correction_parts: [], counts: { scanned_candidates: 0, affected_parents: 0, false_reference_anchors: 0, original_candidates: 0, repaired_candidates: 0, repaired_supported: 0 }, reasons: {} };
  ...
  try {
    for(let i = state.processed_parts.length; i < maxParts; i++) {
      ...
      for await(const line of jsonLines(part.candidates.file)) {
        ...
        group.issues.push(...findAnchorIssues(row.candidate).map(issue => ({ ...issue, original_candidate_hash: row.candidate_hash })));
      }
      finish(); assert.equal(rowCount, part.candidates.rows);
      const rebuiltRows = [], repairedParents = [];
      for(const parent of affected) {
        const staged = await getRaw(parent), before = v1.buildExpandedCandidates(staged), after = v2.buildExpandedCandidates(staged);
        ...
        const repaired = { ...parent, original_parser_part_index: i, repaired_candidate_hashes: after.candidates.map(c => c.candidate_hash), parent_outcome: after.parent_outcome, residuals: after.residuals, review_status: 'PENDING_REVIEW_NOT_PUBLICATION_AUTHORITY' };
        ...
        state.counts.affected_parents++; state.counts.false_reference_anchors += parent.issues.length;
        ...
        for(const issue of parent.issues) state.reasons[issue.reason] = (state.reasons[issue.reason] || 0) + 1;
      }
      if(affected.length) {
...
module.exports = { findAnchorIssues, run };
