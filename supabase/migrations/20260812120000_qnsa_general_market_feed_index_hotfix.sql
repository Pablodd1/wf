...
CREATE INDEX IF NOT EXISTS idx_staging_qnsa_market_feed_global_page
ON staging.listings (normalization_run_key, created_at DESC, id DESC)
WHERE parent_id IS NULL
  AND COALESCE(is_bundle, false) = false
  AND upper(COALESCE(listing_type, intent, '')) IN ('WTS', 'WTB');
...
AS $$
  ...
  WHERE l.category = ANY(c.enabled_categories)
...
GRANT EXECUTE ON FUNCTION public.qnsa_market_feed_counts() TO service_role, postgres;
