...
async function updateVerdicts() {
  ...
  for (const update of updates) {
    try {
      ...
      if (resp.ok) {
        ...
        failed++;
        if (failed <= 5) {
          console.error(`Failed ${update.id}:`, resp.status, await resp.text());
        }
      }
    } catch (e) {
      failed++;
      if (failed <= 5) {
        console.error(`Error ${update.id}:`, e.message);
      }
    }
  ...
  fs.writeFileSync(`update_summary_${timestamp}.json`, JSON.stringify({
