 ...
 for(const [container,database] of [['supabase_db_wf-final-disposable','postgres'],['wf-final-disposable-pg18','wf_production_forward_20260907']]){
  ...
  try{
   ...
   await db.query(`INSERT INTO wf_canonical_staging.mariadb_raw_import_checkpoints(run_key,last_created_on,last_source_id,input_rows,newly_staged_rows,status,frozen_upper_boundary,manifest_sha256,updated_at)
    VALUES($1,'2026-09-03','zzzz',6,6,'RAW_STAGED','{"created_on":"2026-09-03","source_id":"zzzz","count":6}',$2,now())`,[job,sha(job)]);
   await value('SELECT public.create_frozen_normalization_job_v2($1,$1,$2,$1,$3,$4,6) result',[job,sha(job),'disposable','auctions']);
   const claims=await value('SELECT public.claim_normalization_batch_v2($1,$2,6) result',[job,lease]);
   ...
   const oldBatch=await value('SELECT public.publish_materialized_batch_v2($1,$2,$3,true) result',[job+'-OLD',await revision(),old.map(x=>x.materialization_hash)]);assert.equal(oldBatch.inserted,6);
   ...
   const changed=[];for(let i=0;i<5;i++)changed.push(await materialize(records[i].id));assert.deepEqual(changed.map(x=>x.outcome),['ELIGIBLE','REVIEW','ELIGIBLE','ELIGIBLE','REVIEW']);
   assert.deepEqual(await materialize(records[5].id),{...old[5],inserted:0,identical:1});
   const corrected=await value('SELECT public.publish_materialized_batch_v2($1,$2,$3,true) result',[job+'-CORRECTED',await revision(),changed.filter(x=>x.outcome==='ELIGIBLE').map(x=>x.materialization_hash)]);assert.equal(corrected.changed,3);assert.equal(corrected.inserted,0);
   ...
   report.databases.push({container,database,status:'PASS',fixtures:6,corrected:3,withdrawn_review:2,unrelated_unchanged:1,
    checks:['actual normalizer and materializer with immutable proposals','combined Discovery/case groups/brand alias/identity migrations','Datejust correction appears under Rolex browse and Discovery filter; frozen source brand remains Datejust','shared exact reviewed reference and source hash proof','whitespace and punctuation preserved','unreviewed year ranges held; approved exact reviewed references publish','year ranges, substring tokens, changed source, stale materializations rejected','three changed publication records and two exact-ID withdrawals','counts/revision and both current snapshots reconciled','raw/proposal/member/prior versions and frozen snapshot members unchanged','owner-only sidecar; identical replay and changed replay refusal','exact before/after rollback ledger restores all affected records']});
