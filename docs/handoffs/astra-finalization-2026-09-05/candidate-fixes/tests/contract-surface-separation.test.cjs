...
function mockPriceResearchRpc({ wtsRows = [], statsRow = null, wtbRows = [] } = {}) {
  mockRpcHandler = async (name, params = {}) => {
    ...
    if (name === "get_price_research_cohort_breakdown_v2") {
      return {
        data: [{
          total_listings: wtsRows.length + wtbRows.length,
          wts_count: wtsRows.length,
          wtb_count: wtbRows.length,
          qualified_wts_count: statsRow ? statsRow.qualified_count : 0,
          retained_audit_evidence_count: wtsRows.length,
    ...
    if (name === "get_price_research_scoped_stats_v2") {
    ...
    if (name === "open_trading_floor_keyset_snapshot") return { data: "00000000-0000-4000-8000-000000000003", error: null };
    const keysetWrap = (r) => ({
      k_priced_rank: r.priced_rank ?? 2,
      k_image_rank: r.image_rank ?? 2,
      k_price_usd: r.price_usd ?? null,
