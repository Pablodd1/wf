...
-- Repair only the exact-reference transport contract. No listing, raw row,
-- release switch, price, identity, image, or pagination state is changed.
-- The server-side API is the sole public caller; raw phone evidence remains
-- private unless the source listing has explicit publication consent.
...
COMMENT ON FUNCTION public.qnsa_trading_floor_reference_rows(text,text,boolean,integer,integer)
  IS 'Server-only exact-reference feed. Source phones require explicit listing contact consent; source_posted_at_text is preserved without parsing or inference.';
