-- Retain historical versions without treating a sealed snapshot's exact member as ambiguous.
BEGIN;
SET LOCAL lock_timeout='5s';
CREATE OR REPLACE FUNCTION wf_canonical_staging.materialize_single_member_v2(p_job_name text,p_raw_row_id uuid,p_proposal_hash text,p_fx_hash text,p_image_hash text)
...
BEGIN
 ...
 ELSIF EXISTS(SELECT 1 FROM wf_canonical_staging.mariadb_raw_source_rows conflict
   WHERE conflict.source_system=r.source_system AND conflict.source_database=r.source_database AND conflict.source_table=r.source_table
    AND conflict.source_id=r.source_id AND conflict.source_hash<>r.source_hash)
  AND NOT EXISTS (
   -- Membership is created only after every chunk is content-verified and sealed.
   -- These private job/member tables have no public or service-role write grants.
   -- Retained versions outside that exact snapshot are historical evidence.
   ...
   WHERE j.job_name=m.job_name AND s.sealed AND j.capture_run_key IS NULL
    AND j.manifest_sha256=s.manifest_sha256
    AND j.expected_rows=(s.manifest->>'rows')::bigint
