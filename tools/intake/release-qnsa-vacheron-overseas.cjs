...
const RELEASE_RUN_KEY = 'vacheron-overseas-20260819-v1';
const EXPECTED_COUNT = 2500;
const EXPECTED_PLAN_SHA256 = 'ab6575947f568c47306883449e14659444411b76754731d388e13198d73b3c8d';
const MIGRATION = path.join(__dirname, '..', '..', 'supabase', 'migrations',
  '20260820010000_qnsa_vacheron_overseas_release.sql');
...
function catalogReferenceKeys() {
  ...
  const keys = [...new Set((catalog.entries || [])
    .filter(entry => entry.brand === 'Vacheron Constantin' && entry.model === 'Overseas')
    .map(entry => String(entry.normalized_reference || ''))
    .filter(key => /^[A-Z0-9]{3,50}$/.test(key)))].sort();
  if (keys.length < 100) throw new Error('Overseas catalog reference set is incomplete.');
  return keys;
}
...
WITH control AS MATERIALIZED (
  ...
  SELECT s.*,
    (lower(btrim(COALESCE(s.model_normalized, ''))) = 'overseas'
      OR lower(btrim(COALESCE(s.model_original, ''))) = 'overseas'
      OR COALESCE(s.raw_message_text, '') ~* '(^|[^[:alnum:]])overseas([^[:alnum:]]|$)') AS source_overseas_identity,
    regexp_replace(upper(COALESCE(s.reference_normalized, '')), '[^A-Z0-9]', '', 'g')
      = ANY(ARRAY[${catalogArray}]::text[]) AS catalog_overseas_reference,
    upper(COALESCE(s.listing_type, s.intent, '')) AS source_intent
  FROM source s
), cohort AS MATERIALIZED (
  SELECT * FROM marked WHERE source_overseas_identity OR catalog_overseas_reference
  ...
  WHERE source_intent = 'WTS' AND catalog_overseas_reference
...
SELECT id::text AS listing_id, source_hash, source_candidate_hash, release_order,
  CASE
    WHEN catalog_overseas_reference THEN NULLIF(btrim(reference_normalized), '')
  ...
  CASE WHEN catalog_overseas_reference THEN 'CATALOG_OVERSEAS_REFERENCE'
    ELSE 'SOURCE_OVERSEAS_IDENTITY' END AS identity_source,
  catalog_overseas_reference,
...
async function loadPlan() {
  ...
  const plan = (rows || []).map(row => ({
    ...
    catalog_reference_confirmed: row.catalog_overseas_reference === true,
    listing_type: String(row.listing_type),
    price_lane: String(row.price_lane),
    source_overseas_identity: row.source_overseas_identity === true,
...
function selectCanary(plan) {
  const predicates = [
    ...
    row => row.identity_source === 'SOURCE_OVERSEAS_IDENTITY',
...
async function ensureRun(plan, planSha) {
  ...
  await managementQuery(`BEGIN;
    INSERT INTO public.qnsa_vacheron_overseas_release_runs(
    ...
    ON CONFLICT (release_run_key) DO UPDATE SET updated_at = now()
      WHERE qnsa_vacheron_overseas_release_runs.plan_sha256 = EXCLUDED.plan_sha256
        AND qnsa_vacheron_overseas_release_runs.planned_count = EXCLUDED.planned_count
        AND qnsa_vacheron_overseas_release_runs.release_mode <> 'ROLLED_BACK';
    DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM public.qnsa_vacheron_overseas_release_runs
  ...
  const rows = await managementQuery(`BEGIN;
    WITH payload AS (
      SELECT * FROM jsonb_to_recordset('${json}'::jsonb) AS x(
        ...
        public_reference text, identity_source text, catalog_reference_confirmed boolean, price_lane text)
    )
    INSERT INTO public.qnsa_vacheron_overseas_release_manifest(
      listing_id, release_run_key, release_order, source_hash, source_candidate_hash,
      public_reference, identity_source, catalog_reference_confirmed, price_lane)
    SELECT listing_id, ${sqlText(RELEASE_RUN_KEY)}, release_order, source_hash, source_candidate_hash,
      public_reference, identity_source, catalog_reference_confirmed, price_lane FROM payload
    ON CONFLICT (listing_id) DO UPDATE SET
      ...
      public_reference=EXCLUDED.public_reference, identity_source=EXCLUDED.identity_source,
      catalog_reference_confirmed=EXCLUDED.catalog_reference_confirmed, price_lane=EXCLUDED.price_lane;
    SELECT count(*)::integer AS reconciled FROM public.qnsa_vacheron_overseas_release_manifest
...
async function activate(mode, expectedCount, planSha) {
  const rows = await managementQuery(`BEGIN;
    UPDATE public.qnsa_vacheron_overseas_release_control SET enabled=true,
    ...
    UPDATE public.qnsa_vacheron_overseas_release_runs SET release_mode=${sqlText(mode)}, updated_at=now()
    ...
    FROM public.qnsa_vacheron_overseas_release_manifest
    WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
  COMMIT;`);
...
async function rollback() {
  const rows = await managementQuery(`BEGIN;
    UPDATE public.qnsa_vacheron_overseas_release_control SET enabled=false, release_run_key=NULL,
    ...
    DELETE FROM public.qnsa_vacheron_overseas_release_manifest WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
    UPDATE public.qnsa_vacheron_overseas_release_runs SET release_mode='ROLLED_BACK', updated_at=now()
      WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
    SELECT count(*)::integer AS remaining FROM public.qnsa_vacheron_overseas_release_manifest
      WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
  COMMIT;`);
...
async function main() {
  ...
  const output = path.resolve(arg('output', path.join('audit-output', 'vacheron-overseas-release-report.json')));
  ...
  const confirmation = String(process.env.VACHERON_OVERSEAS_RELEASE_CONFIRMATION || '');
  const required = mode === 'audit' ? 'AUDIT_QNSA_VACHERON_OVERSEAS_RELEASE'
    : mode === 'canary' ? 'APPLY_QNSA_VACHERON_OVERSEAS_CANARY'
    : mode === 'full' ? 'APPLY_QNSA_VACHERON_OVERSEAS_FULL'
    : 'ROLLBACK_QNSA_VACHERON_OVERSEAS_RELEASE';
  ...
  if (mode === 'rollback') {
    ...
    planSummary = {
      source_expected: 1493,
      source_found: 2972,
      unique_individual_listings: plan.length,
      duplicates_excluded: 472,
