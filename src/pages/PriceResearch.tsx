...
function PriceRangeBar({ min, avg, max }: { min: number; avg: number; max: number }) { const r = max-min||1; const ap = ((avg-min)/r)*100; return <div className="w-full"><div className="relative h-16"><div className="absolute top-8 left-0 right-0 h-1 bg-gray-200 rounded-full" /><div className="absolute" style={{left:'0%',top:'18px'}}><div className="flex flex-col items-center"><span className="text-xs text-gray-400 uppercase tracking-wider mb-1">Min</span><div className="w-3.5 h-3.5 rounded-full bg-gray-400 border-2 border-white shadow-md"/><span className="text-xs text-gray-600 font-mono mt-1 font-medium">{fmtPrice(min)}</span></div></div><div className="absolute" style={{left:`${ap}%`,top:'12px',transform:'translateX(-50%)'}}><div className="flex flex-col items-center"><span className="text-xs text-blue-600 font-bold uppercase tracking-wider mb-1">Average</span><div className="w-7 h-7 rounded-full bg-[#3B5BFE] border-[3px] border-white shadow-lg"/><span className="text-sm text-[#3B5BFE] font-mono font-bold mt-1">{fmtPrice(avg)}</span></div></div><div className="absolute" style={{right:'0%',top:'18px'}}><div className="flex flex-col items-center"><span className="text-xs text-gray-400 uppercase tracking-wider mb-1">Max</span><div className="w-3.5 h-3.5 rounded-full bg-gray-400 border-2 border-white shadow-md"/><span className="text-xs text-gray-600 font-mono mt-1 font-medium">{fmtPrice(max)}</span></div></div></div></div>; }
        ...
        {result && !loading && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              {[{label:'Average',val:fmtPrice(result.overallAvg),c:'text-[#3B5BFE]'},{label:'Median',val:fmtPrice(result.medianPrice),c:'text-gray-800'},{label:'Range',val:`${fmtPrice(result.overallMin)}-${fmtPrice(result.overallMax)}`,c:'text-gray-800'},{label:'Listings',val:`${result.totalListings}`,c:'text-gray-800'},{label:'Drift',val:`${result.priceDrift>0?'+':''}${result.priceDrift}%`,c:result.priceDrift<0?'text-red-500':'text-green-500',icon:result.priceDrift<0?<TrendingDown size={16}/>:<TrendingUp size={16}/>}].map(s => <div key={s.label} className="bg-white rounded-xl p-4 text-center border border-gray-200 shadow-sm"><div className="text-xs text-gray-500 uppercase tracking-wider mb-1">{s.label}</div><div className={`text-lg font-bold ${s.c} flex items-center justify-center gap-1`}>{s.icon}{s.val}</div></div>)}
            </div>
