...
const PARSER_VERSION = 'expanded-evidence-v7-explicit-source-repair';
...
const referenceValidationFile = path.resolve(__dirname, 'expanded-explicit-reference-validation-v7.json');
const catalog = JSON.parse(fs.readFileSync(catalogFile));
const referenceValidation = JSON.parse(fs.readFileSync(referenceValidationFile));
...
const DEPENDENCY_HASHES = Object.freeze(Object.fromEntries([
  ['explicit_reference_validation', referenceValidationFile],
  ['previous_inventory_parser', path.resolve(__dirname, 'expanded-evidence-candidates-v6-r2.cjs')],
...
for (const entry of [...catalog.entries, ...referenceValidation.entries]) {
...
function parsingView(original) {
  ...
  points.forEach((point, index) => {
    const derived = point.normalize('NFKC').replace(/\p{Cf}/gu, '').replace(/[\uFE0F\u20E3]/g, '');
    if (!derived && /^[\uFE0F\u20E3]$/.test(point) && ends.length) ends[ends.length-1]=index+1;
    text += derived;
...
function priceSourceRole(text, start, end) {
  ...
  const labels=[...linePrefix.matchAll(/\b(?:retail(?:\s+price)?|rrp|msrp|list\s+price|budget|paid|cost|trade\s+value|insurance|valuation|asking(?:\s+price)?|sale\s+price|net\s+price|yours\s+for|price|ask|firm|obo)\b/gi)];
  const label=labels.at(-1)?.[0]||'';
  const role=supplemental?'NON_ASK':/^budget$/i.test(label)?'EXPLICIT_BUDGET':nonAskPattern.test(label)?'NON_ASK':label||/\b(?:firm|obo)\b/i.test(after.slice(0,16))?'EXPLICIT_ASK':'UNLABELLED';
  return {role,adjustment,explicit_sale_role:role==='EXPLICIT_ASK'&&/\b(?:asking|sale\s+price|yours\s+for)\b/i.test(label)};
}
...
function prices(text, currency = null) {
  ...
  for (const p of claimed) {
    ...
    const hasScale = /(?:^|[\d.,\s])(?:million|mill|mil|mn|m|k)(?=$|[^a-z]|(?:usd|usdt|hkd|eur|gbp|chf|sgd|jpy|cny|rmb|aud|cad|nzd|inr|thb|myr|krw|twd|sar|qar|kwd|bhd|zar|brl|mxn|php|idr|vnd|try|dkk|nok|sek)\b)/i.test(quote);
...
function exactDecimalAmount(rawNumber, hadMultiplier, quote) {
  ...
  const scaleMatch = /(?:^|[\d.,\s])(million|mill|mil|mn|m|k)(?=$|[^a-z]|(?:usd|usdt|hkd|eur|gbp|chf|sgd|jpy|cny|rmb|aud|cad|nzd|inr|thb|myr|krw|twd|sar|qar|kwd|bhd|zar|brl|mxn|php|idr|vnd|try|dkk|nok|sek)\b)/i.exec(quote);
...
function referenceClaims(text, brand, rawReference, priceFacts) {
  ...
  const tokenPattern = /RM[ \t]*\d{2,3}[ \t]*-[ \t]*\d{2}\b|(?:RM|PAM|G0A)[ \t]+\d[A-Z0-9]*(?:[./-][A-Z0-9]+)*|\d{5,6}[ \t]+(?:LN|LB|LV|BLRO|BLNR|CHNR|GRNR|RBOW)\b|[A-Z0-9]+(?:[./-][A-Z0-9]+)*/gi;
  for (const m of text.matchAll(tokenPattern)) {
    ...
    if (/(?:price|asking|ask|msrp|rrp|retail|budget|paid|cost|sku|item\s*(?:id|no|number)|stock\s*(?:id|code|no|number)?|serial\s*(?:id|no)?|s\/n|tel|phone|whatsapp)\s*[:#=-]?\s*$/i.test(text.slice(Math.max(0, start - 24), start))) continue;
...
function buildExpandedCandidates(staged) {
  ...
  for (const line of lines) {
    ...
    const condition = isDecoratedCondition(line.text) || (!refs.length && !ps.length && bs.length === 1 ? isDecoratedCondition(line.text.replace(makerPattern,'')) : null);
    ...
    if (!refs.length && (onlyMaker || curr || condition || onlyIntent || structured)) {
      finish();
      if ((onlyMaker || (structured && bs.length === 1)) && context.brand?.value && !sameBrand(context.brand.value,bs[0])) context.condition = null;
  ...
  for (const [index, block] of blocks.entries()) {
    ...
    const partOnly = /\b(?:mid\s+(?:\w+\s+){0,2}case|(?:case\s*back|bracelet|dial|bezel|movement)\s+(?:only|for\s+sale))\b/i.test(text)
      || (ref && sourceLines(text).some(line=>new RegExp('^(?:(?:Rolex|WTS|FS)\\s+)*'+escape(ref.reference)+'\\s+(?:case|back)(?:\\s+only)?[.! ]*$','i').test(line.text)));
    if (partOnly) { reasons.push('NONWATCH_PARTS_OFFER'); evidence.accessory_scope=[ev(0,text.length,'NONWATCH_PARTS_OFFER')]; }
    fields.category = partOnly ? 'ACCESSORY' : 'WATCH';
    ...
    const ownAvailability = /\b(sold|hold|reserved)\b(?!\s+order\b)/i.exec(text);
    if (ownAvailability && /^(?:open|active|available)$/i.test(envelope.source_status || '')) {
      const ownLine=text.slice(0,ownAvailability.index).split(/[\r\n\u2028\u2029]/).at(-1);
      const remainder=text.slice(ownAvailability.index+ownAvailability[0].length).split(/[\r\n\u2028\u2029]/)[0];
      const negated=/\b(?:not|never|no|unsold|can|cannot|will|previously|formerly|was)\b/i.test(ownLine);
      const marked=/^[\s*)\]!.:;-]*$/.test(remainder)&&(!ownLine.trim()||/[([]\s*$/.test(ownLine)||/\b(?:on|now)\s+$/i.test(ownLine));
      if (!negated && marked) {
        fields.source_status=ownAvailability[1].toLowerCase();
        evidence.source_status=[ev(ownAvailability.index,ownAvailability.index+ownAvailability[0].length,'EXPLICIT_OWN_OFFER_AVAILABILITY')];
      } else reasons.push('OWN_AVAILABILITY_CONTEXT_REQUIRES_REVIEW');
    }
    if (ownAvailability && ownAvailability[1].toLowerCase()==='sold' && fields.intent==='WTB') reasons.push('BUY_INTENT_VERSUS_SOLD_OFFER_REVIEW');
