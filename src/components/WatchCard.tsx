...
import { Star, Package, Paperclip, ArrowRight, MessageCircle } from 'lucide-react';
...
export type WatchCardRecord = ListingDisplayContract | (Partial<ListingDisplayContract> & WatchRecord);
...
export function WatchCard({ record, index, onSelect }: WatchCardProps) {
  ...
  const r = record as Partial<ListingDisplayContract> & Record<string, any>;
  ...
  if (r.price_status === 'VERIFIED_USD' && r.price_usd !== null && r.price_usd !== undefined) {
    ...
    priceDisplay = `$${Number(r.price).toLocaleString()}`;
