...
def process_brand(brand_key, sql_query, output_prefix, canon_brand_name, processor, conn):
    print(f"\n==================================================", flush=True)
    print(f"Starting processing for {canon_brand_name}...", flush=True)
    print(f"==================================================", flush=True)
    ...
    print(f"Retrieved {len(rows)} records for {canon_brand_name}. Ingesting and building admission decisions...", flush=True)
    ...
    for idx, r in enumerate(rows):
        ...
        if isinstance(created_on, datetime):
            ...
            source_posted_at = "2026-08-16T15:34:00+00:00"
        ...
        source_brand_text = r.get('brand') or canon_brand_name
        ...
        # -------------------------------------------------------------
        # SHEET 2 ADMISSION DECISIONS
        # -------------------------------------------------------------
        final_brand = canon_brand_name

        # Model mapping
        model_norm = res.get('model_normalized') or source_model_text or ""
        if canon_brand_name == "TAG Heuer":
            if model_norm in ("", "Unspecified", "None", "0"):
                if 'carrera' in msg_norm: model_norm = "Carrera"
                elif 'monaco' in msg_norm: model_norm = "Monaco"
                elif 'aquaracer' in msg_norm: model_norm = "Aquaracer"
                elif 'formula 1' in msg_norm or 'formula1' in msg_norm or 'f1' in msg_norm: model_norm = "Formula 1"
                elif 'autavia' in msg_norm: model_norm = "Autavia"
                elif 'link' in msg_norm: model_norm = "Link"
                elif 'monza' in msg_norm: model_norm = "Monza"
                elif 'connected' in msg_norm: model_norm = "Connected"
                else: model_norm = "TAG Heuer Collection"
        elif canon_brand_name == "Breguet":
            if model_norm in ("", "Unspecified", "None", "0"):
                if 'marine' in msg_norm: model_norm = "Marine"
                elif 'classique' in msg_norm: model_norm = "Classique"
                elif 'type xx' in msg_norm or 'type xxi' in msg_norm or 'type xxii' in msg_norm or 'type 20' in msg_norm: model_norm = "Type XX / XXI / XXII"
                elif 'reine de naples' in msg_norm: model_norm = "Reine de Naples"
                elif 'tradition' in msg_norm: model_norm = "Tradition"
                elif 'heritage' in msg_norm or 'héritage' in msg_norm: model_norm = "Héritage"
                else: model_norm = "Breguet Collection"
        ...
        reviewed_at = "2026-08-16T15:34:00+00:00"
    ...
    print(f"Building Multi-Sheet Workbook for {canon_brand_name} ({len(rows)} records)...", flush=True)
    ...
    # Sheet 2 Setup
    sheet2_title = f"{canon_brand_name} Admissions" if len(f"{canon_brand_name} Admission Decisions") > 31 else f"{canon_brand_name} Admission Decisions"
    ...
    master_filename = f"{output_prefix}_Trading_Floor_Admission_Master.xlsx"
    wb.save(master_filename)
    print(f"Workbook successfully saved to {master_filename}", flush=True)

    # Copy to user locations
    dest_downloads = os.path.expanduser(f"~/Downloads/{master_filename}")
    dest_desktop = os.path.expanduser(f"~/Desktop/{master_filename}")
    shutil.copyfile(master_filename, dest_downloads)
    shutil.copyfile(master_filename, dest_desktop)

    # Also update Normalized_Master_Inventory file
    norm_filename = f"{output_prefix}_Normalized_Master_Inventory.xlsx"
    shutil.copyfile(master_filename, norm_filename)
    shutil.copyfile(master_filename, os.path.expanduser(f"~/Downloads/{norm_filename}"))
    shutil.copyfile(master_filename, os.path.expanduser(f"~/Desktop/{norm_filename}"))
...
def main():
    ...
    tag_sql = """
        ...
        FROM auctions a
        WHERE a.brand IN ('TAG Heuer', 'Tag Heuer', 'TAG HEUER', 'Tag heuer', 'Heuer', 'TAG')
           OR a.title LIKE '%TAG Heuer%'
           OR a.title LIKE '%Tag Heuer%'
           OR a.title LIKE '%Carrera%'
           OR a.title LIKE '%Monaco%'
           OR a.title LIKE '%Aquaracer%'
        ORDER BY a.created_on ASC, a.id ASC;
    """
    process_brand("tag", tag_sql, "TAG_Heuer", "TAG Heuer", processor, conn)
    ...
    breguet_sql = """
        SELECT a.id, a.open_unique_key, a.title, a.description, a.front_image, a.created_on, a.type,
               ...
               a.is_seller_approved, a.company_id, a.phone_code, a.category_id, a.number
        FROM auctions a
        WHERE a.brand IN ('Breguet', 'breguet', 'BREGUET', 'Breguet ')
           OR a.title LIKE '%Breguet%'
           OR a.description LIKE '%Breguet%'
           OR a.title LIKE '%Reine de Naples%'
           OR a.title LIKE '%Classique%'
           OR a.title LIKE '%Tradition%'
        ORDER BY a.created_on ASC, a.id ASC;
    """
    process_brand("breguet", breguet_sql, "Breguet", "Breguet", processor, conn)

    conn.close()
    print("\nALL BRANDS (TAG HEUER & BREGUET) PROCESSED AND DELIVERED SUCCESSFULLY!", flush=True)
