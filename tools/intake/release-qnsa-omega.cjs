...
const RELEASE_RUN_KEY = 'omega-20260820-v1';
const EXPECTED_COUNT = 6871;
const EXPECTED_PLAN_SHA256 = '5af3fde0cc6b2098288054de89fe5ac7e3e77f979c4bf3716eb4375c4a8e6595';
const MIGRATION = path.join(__dirname, '..', '..', 'supabase', 'migrations',
  '20260820130000_qnsa_omega_release.sql');
...
function catalogReferenceKeys() {
  ...
  if (keys.length < 100) throw new Error('Omega catalog reference set is incomplete.');
  return keys;
}
...
function catalogModelsByReference() {
  ...
  return new Map((catalog.entries || [])
    .filter(entry => entry.brand === 'Omega' && entry.normalized_reference && entry.model)
...
WITH control AS MATERIALIZED (
  ...
  SELECT s.*,
    COALESCE(s.raw_message_text, '') ~* '(^|[^[:alnum:]])omega([^[:alnum:]]|$)' AS raw_omega_brand,
  ...
  WHERE source_intent = 'WTS' AND catalog_omega_reference
...
SELECT id::text AS listing_id, source_hash, source_candidate_hash, release_order,
  CASE
    WHEN catalog_omega_reference THEN NULLIF(btrim(reference_normalized), '')
  ...
  CASE WHEN catalog_omega_reference THEN 'CATALOG_OMEGA_REFERENCE'
    ELSE 'SOURCE_OMEGA_IDENTITY' END AS identity_source,
  catalog_omega_reference,
  ...
  END AS price_lane,
  (raw_omega_brand OR raw_omega_collection) AS source_omega_identity,
...
async function loadPlan() {
  ...
  const plan = (rows || []).map(row => ({
    ...
    public_model: catalogModels.get(String(row.public_reference || '').replace(/[^A-Za-z0-9]/g, '').toUpperCase()) || 'Omega',
    identity_source: String(row.identity_source),
    catalog_reference_confirmed: row.catalog_omega_reference === true,
    listing_type: String(row.listing_type),
    price_lane: String(row.price_lane),
    source_omega_identity: row.source_omega_identity === true,
...
function selectCanary(plan) {
  const predicates = [
    ...
    row => row.identity_source === 'SOURCE_OMEGA_IDENTITY',
...
async function ensureRun(plan, planSha) {
  ...
  await managementQuery(`BEGIN;
    INSERT INTO public.qnsa_omega_release_runs(
    ...
    ON CONFLICT (release_run_key) DO UPDATE SET updated_at = now()
      WHERE qnsa_omega_release_runs.plan_sha256 = EXCLUDED.plan_sha256
        AND qnsa_omega_release_runs.planned_count = EXCLUDED.planned_count
        AND qnsa_omega_release_runs.release_mode <> 'ROLLED_BACK';
    DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM public.qnsa_omega_release_runs
...
async function insertBatch(batch) {
  ...
  const rows = await managementQuery(`BEGIN;
    ...
    INSERT INTO public.qnsa_omega_release_manifest(
    ...
    SELECT count(*)::integer AS reconciled FROM public.qnsa_omega_release_manifest
...
async function activate(mode, expectedCount, planSha) {
  const rows = await managementQuery(`BEGIN;
    UPDATE public.qnsa_omega_release_control SET enabled=true,
    ...
    UPDATE public.qnsa_omega_release_runs SET release_mode=${sqlText(mode)}, updated_at=now()
    ...
    FROM public.qnsa_omega_release_manifest
    WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
  COMMIT;`);
...
async function rollback() {
  const rows = await managementQuery(`BEGIN;
    UPDATE public.qnsa_omega_release_control SET enabled=false, release_run_key=NULL,
    ...
    DELETE FROM public.qnsa_omega_release_manifest WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
    UPDATE public.qnsa_omega_release_runs SET release_mode='ROLLED_BACK', updated_at=now()
      WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
    SELECT count(*)::integer AS remaining FROM public.qnsa_omega_release_manifest
      WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
  COMMIT;`);
...
async function main() {
  ...
  const output = path.resolve(arg('output', path.join('audit-output', 'omega-release-report.json')));
  ...
  const confirmation = String(process.env.OMEGA_RELEASE_CONFIRMATION || '');
  const required = mode === 'audit' ? 'AUDIT_QNSA_OMEGA_RELEASE'
    : mode === 'canary' ? 'APPLY_QNSA_OMEGA_CANARY'
    : mode === 'full' ? 'APPLY_QNSA_OMEGA_FULL'
    : 'ROLLBACK_QNSA_OMEGA_RELEASE';
  ...
  if (mode === 'rollback') {
    ...
    planSummary = {
      source_expected: 10384,
      source_found: 10384,
      identity_held: 803,
      unique_individual_listings: plan.length,
      duplicates_excluded: 2710,
