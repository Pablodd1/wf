...
const script = fs.readFileSync(path.join(root, 'tools', 'audit', 'qnsa-omega-release-census.cjs'), 'utf8');
const workflow = fs.readFileSync(path.join(root, '.github', 'workflows', 'qnsa-omega-release-census.yml'), 'utf8');

test('Omega census is QNSA-pinned, aggregate-only, and read-only', () => {
  ...
  assert.match(workflow, /AUDIT_QNSA_OMEGA_RELEASE/);
  assert.match(workflow, /environment: production/);
  assert.match(workflow, /permissions:\s*\n\s*contents: read/);
});

test('Omega census separates identity, intent, price, media, dealer, and missing fields', () => {
  for (const marker of [
