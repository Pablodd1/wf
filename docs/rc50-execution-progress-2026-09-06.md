...
# 2026-09-07 12:04 UTC — Trading Floor priority and production preparation

The owner approved proceeding with all eligible single-item Trading Floor cards, retaining the UI. Price Research expansion and bundles follow later. The previously proposed 90-to-135 GB disk increase is approved; Supabase additionally requires disabling the organization's spend cap. That broader billing change remains a separate pending question, and storage has not changed.

Fresh snapshot qualification remains 1,527,898 exact inputs, including 104,160 preliminary Trading Floor candidates. These are private candidates, not published results. The sealed-snapshot materialization correction allows the selected raw version while retaining historical versions and legacy ambiguity holds. Actual PostgreSQL 15 and 18 tests pass selected-content/price preservation, replay, unsealed/wrong-scope rejection, legacy conflict quarantine and private access.

Production was reverified at `a8245646d857ea20ce7f2963db3f53dd8620b2a3`; its functions and columns still match the reviewed schema export. A private pre-mutation rollback capture preserves 500 published rows, zero approved dealers/source identities, and 16,094 lineage rows; SHA-256 `8ee218017b4a911fb2e3d832dbeaeeb8aac145a96e594d44a896692ea080f2c0`. The exact 51-file forward migration plan passes the populated production-shape clone with the existing publication fingerprint unchanged. Production has not yet been mutated by this execution.

# 2026-09-07 17:00 UTC � live source-backed rollout and owner-approved contact

This checkpoint supersedes the older production-preparation entries above. Full completion is not claimed. The owner authorized production publication, the 135 GB disk/spend-cap change, and buyer inquiries to the original source posters. Singles remain the active scope; bundles and multi-listing decomposition are deferred. No generated or replacement images are permitted, and the existing UI layout remains unchanged.

The complete 1,527,898-row frozen source boundary is durably sealed in Supabase. Its manifest SHA-256 is `de3e2387d31e82dc87bad68d0c3128de010e13c86aee58567b0f0ce00803a426`. Three normalization workers and one progressive publisher continue through that boundary. Full materialization/reconciliation waits for normalization completion. Over 650,000 source inputs have been processed; this is a processing count, not a published count.

The live application is `4b22e5a9f8007980510b8348cfddb9bca1cdbf86`, Vercel deployment `dpl_7itf1UQHbEvi6az9XhrsCVJ6PfJM`, project `prj_2Cb6ZB6nfvs3dIJN3Uer9IAFVHZs`, at https://wf-ecru.vercel.app. Promotion completed at 16:38:59 UTC. The preceding source-backed deployment `dpl_4Mppb2d46t1xk8H8doxHnu4ysgh3` remains the application rollback target. The contact change includes the exact selected listing ID in the buyer inquiry; no messages were sent during verification. Owner attestation and the 264-dealer permission mutation have private checksum-bound evidence and a pre-mutation backup. Unverified identities and unavailable reviews remain unresolved rather than invented.

The latest complete live snapshot audits passed at 16:54 UTC: 9,155 Trading Floor cards across 92 pages, with 9,048 original source images, 323 evidenced USD prices, and 1,768 source-linked dealer cards; Price Research contains 288 qualified unique priced WTS observations across three pages. Both audits found zero duplicate or missing rows and checked all 52 contract fields against the frozen database snapshot. Real browser verification passed original photo rendering, the existing contact button, 50 desktop and 24 mobile cards, and Price Research, without changing layout. Source photos can experience external connection resets; successful reruns and failed-run evidence are retained.

A source-identity review found upstream metadata conflicting with the actual message. A tested forward guard now requires message corroboration for brand/reference metadata and omits unsupported model metadata. The initial 5,550 publications were atomically corrected to 4,833 eligible publications plus 717 durable review outcomes, preserving all raw messages and image evidence. Subsequent cohorts use the same guard. Private backup SHA-256: `c913ed11ea58ff0cfc1e8a94c2acc13d9dbd06c34f45c7fbc5a3d8a7164f0bcc`; correction plan SHA-256: `13a8525569be6732ef9f922d7332e6d0ddfc61bb5ff0320ec4d7ffb125f59c2c`. Current cohort rollback keys and exact mutation receipts remain in the local evidence directory.

There are 63 reviewed forward migrations applied to the documented production baseline. Additional PostgreSQL 15/18 checks cover source identity correction, same-frozen-run image receipt reuse, and indexed normalization claims. The indexed query selected identical production UUIDs read-only, reducing warm selection time from 674 ms to 45 ms using existing indexes. HTTP response-body retries and expired lease recovery preserve the existing source validation and retry limit. A bounded direct-SQL transport experiment committed 500 rows before a connection termination; the normal HTTP workers continue from durable state. No full-suite green claim is made; previously documented baseline failures remain classified separately.

Supabase storage was evaluated read-only: 90.32 GB at 15:28 UTC, growing during the active import to 94.43 GB at 16:55 UTC. The largest older workbook relation contains 25.85 GB of indexes. Four non-unique index candidates total 4.96 GB, but zero scan counters do not establish safe removal. No data, table, index, or image was deleted. Raw evidence, review outcomes, publication lineage and rollback archives must remain. The complete local report is `outputs/supabase-storage-evaluation-20260907.md`.

Remaining work is full-boundary normalization, durable materialization of every input, publication of every eligible single, complete Trading Floor/Price Research reconciliation and browser acceptance, exact final deployment evidence, and owned-worker shutdown after completion. There is no pending user permission request.

# 2026-09-07 17:48 UTC � concurrent full reconciliation and expanded publication

The public API reports 22,606 Trading Floor listings and 642 qualified priced Price Research offers on the same live application commit `4b22e5a9f8007980510b8348cfddb9bca1cdbf86`. These are current API totals; the last full snapshot audits remain the separately recorded 9,155/288 audits. A live dealer-profile audit now verifies all 644 source-linked listings across seven pages, including prices, original images, intent, consented contact availability and null unsupported dates/ratings. Price Research's real browser detail check passed its original thumbnail, raw message, profile link and exact item inquiry. The source `auction_ratings` table has exactly zero rows, confirmed read-only.

Full materialization/reconciliation is now active alongside normalization. Forward migration `20260909170000_reconcile_completed_frozen_prefix.sql` allows this only for a completely sealed immutable raw boundary. Its reader stops at the first unfinished input, including gaps before a completed later canary row; its commit rejects pending/leased inputs. The expected count remains 1,527,898 and completion requires every input. Actual PostgreSQL 15/18 worker tests prove waiting, partial-prefix advancement, refusal to skip gaps, full completion, and continued rejection of incomplete legacy captures. The production migration SHA-256 is `85e5c3e8c705819a2f530301dfffcc2b4a12bd385859640931073d401cb7a944`, applied at 17:28:47 UTC. There are now 64 reviewed forward migrations. At 17:43 UTC, normalization had processed 887,550 inputs and reconciliation had durably processed 56,500 with no error outcomes.

Original photo checks now retry a transient transport/service failure once against the identical source URL, retaining the failed receipt before the final receipt. Permanent absence, invalid content and altered source lineage cannot become replacement images. Nine focused image tests and both completed-prefix integration tests pass. A 120-photo production read-only probe at concurrency 24 with bounded retry verified all original images, but showed no consistent throughput advantage over 12; the publisher remains at 12 concurrent probes. During a worker restart, an older in-flight HTTP batch committed before the replacement committed the same prefix. The stale cursor was correctly refused, and the worker resumed from the newer durable database checkpoint. No raw evidence was lost or edited.

The full rollout is still active; no completion claim or new user permission request is made. Bundles and multi-listing decomposition remain deferred, and no Supabase cleanup has been performed.

### September 7, 2026 — 19:20 UTC restart and source-poster checkpoint

The laptop restart preserved the sealed 1,527,898-input Supabase boundary. Read-only production verification after restart confirmed 1,196,550 committed normalization outcomes and 39,786 published singles. The owned workers resumed from database checkpoints. One idle pre-restart materialization session retained advisory lock (724051,9013); its exact PID, backend start, last-query timestamp and idle/no-transaction state were checked before releasing it. No raw evidence was modified.

The live application is a77c5f54a0860f90c179e6db2c3a15264a421fc3, deployment dpl_uQi4CCDwTuiU2MsWLQPve2iftd5M at the existing wf-ecru.vercel.app alias. Original-photo cards, selected-item WTS/WTB inquiries and opaque dealer-profile contact actions passed real API and desktop/mobile checks. Two earlier browser timeout attempts remain preserved; the instrumented repeat passed. No messages were sent. The previous deployment dpl_7itf1UQHbEvi6az9XhrsCVJ6PfJM remains the rollback target.

The owner explicitly reiterated that all original posters want to be contacted and that contact permission must not be asked again. The pending source-poster extension separates exact phone/company linkage from dealer reputation verification. Its private full-boundary plan identifies 3,230 additional poster identities across 83,881 preliminary single candidates. These identities remain UNVERIFIED, with no invented ratings, reviews or verification dates; shared/mismatched phones and restricted companies remain held. Forty focused tests and actual PostgreSQL 15/18 importer/contact/profile/directory tests passed. The migration 20260909180000_exact_source_poster_contacts.sql and associated application changes are reviewed source only at this checkpoint: NOT applied to production or deployed yet. Existing 64 forward migrations remain the production state.

Remaining work: validate the additional poster plan against current production identities, apply the tested forward migration and reviewed bounded import, reconcile existing published links, verify exact application/API/browser integration, and continue every frozen input through durable normalization and materialization. Full-boundary completion is not claimed. Bundles and multi-listing decomposition remain deferred. No generated listing images, storage deletions, new paid resources, or unrelated checkout changes were performed.
