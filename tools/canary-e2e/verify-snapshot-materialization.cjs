...
for(const [container,database] of [['supabase_db_wf-final-disposable','postgres'],['wf-final-disposable-pg18','wf_production_forward_20260907']]){
 ...
 sql('UPDATE wf_canonical_staging.mariadb_raw_source_rows SET source_created_on=\'2001-01-01T00:00:00Z\',captured_at=\'2001-01-02T00:00:00Z\' WHERE id='+literal(selected.id)+';');
 ...
 manifest.started_at='2026-09-07T01:00:00.000Z';
 ...
 assert.equal(members[0].raw.source_created_on,null);assert.equal(members[0].raw.captured_at,manifest.started_at);
