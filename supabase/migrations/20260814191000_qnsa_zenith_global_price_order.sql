-- Stable Zenith customer pagination: exact source images first, then verified
-- USD prices, then genuine no-price activity. The reconciled identity ledger is
-- an explicit admission dependency; pagination occurs only after every gate.

BEGIN;

CREATE OR REPLACE FUNCTION public.qnsa_zenith_ordered_candidate_page(
...
BEGIN
  ...
  WITH eligible AS MATERIALIZED (
    SELECT l.id,
      (btrim(COALESCE(l.image_url,l.source_media_url_candidate,'')) ~* '^https?://[^[:space:]]+$') AS has_image,
      (l.price_usd>0 AND (l.currency_normalized IN ('USD','USDT')
        OR (l.conversion_rate>0 AND l.conversion_timestamp IS NOT NULL))) AS has_price,
      l.reference_normalized AS sort_reference
    ...
    JOIN staging.qnsa_zenith_identity_reconciliation_audit a
     ...
     AND a.reconciliation_run_key='zenith-identity-20260814-v1'
     AND a.decision='RELEASE_SAFE' AND a.corrected_reference=l.reference_normalized
    ...
    WHERE l.normalization_run_key=v_run_key AND l.brand_normalized='Zenith'
    ...
    ORDER BY has_image DESC,has_price DESC,l.reference_normalized ASC,l.id ASC
    LIMIT v_limit+1 OFFSET v_offset
  ), selected AS MATERIALIZED (
    SELECT * FROM eligible
    ORDER BY has_image DESC,has_price DESC,sort_reference ASC,id ASC LIMIT v_limit
  ), contracted AS MATERIALIZED (
    SELECT s.has_image,s.has_price,s.sort_reference,l.id AS sort_id,jsonb_build_object(
      'id',l.id::text,'parent_id',l.parent_id::text,'source_file','MARIADB_IMMUTABLE_RAW',
      'source_row_number',1,'source_record_id',l.source_record_id,'posting_date',l.created_at,
      ...
      'raw_message',l.raw_message_text,'listing_type',upper(COALESCE(l.listing_type,l.intent,'')),
      'brand_scope',l.brand_normalized,'supplied_brand',l.brand_original,'canonical_brand',l.brand_normalized,
      'model',l.model_original,'catalog_model',l.model_normalized,
      'raw_reference',l.reference_original,'normalized_reference',l.reference_normalized,
      'catalog_reference',l.reference_normalized,'dial_color',l.dial_color_normalized,
      'catalog_dial',l.dial_color_normalized,'condition',l.condition_normalized,
      'workbook_price_usd',l.price_usd,'source_price_amount',l.price_normalized,
      ...
      'price_evidence_status',CASE
        ...
        WHEN l.price_normalized>0 THEN 'CURRENCY_UNCONFIRMED' ELSE 'PRICE_NOT_SUPPLIED' END,
      'confidence',l.overall_confidence,'verdict',l.verdict,'verification_status',l.verdict,
      'user_image_url',CASE WHEN btrim(COALESCE(l.image_url,l.source_media_url_candidate,'')) ~* '^https?://[^[:space:]]+$'
        THEN btrim(COALESCE(l.image_url,l.source_media_url_candidate)) END,
      'imported_at',l.created_at,'has_exact_source_image',s.has_image,
      'verified_price_usd',CASE WHEN s.has_price THEN l.price_usd END,
      'has_verified_usd_price',s.has_price,'has_complete_identity',true,
      'trading_floor_status',l.trading_floor_status,
      'reference_search_key',regexp_replace(upper(l.reference_normalized),'[^A-Z0-9]','','g'),
      'location',COALESCE(NULLIF(btrim(l.location),''),NULLIF(btrim(rv.raw_payload#>>'{raw_data,region}'),'')),
      'item_category','WATCH','publication_state',CASE
        WHEN upper(COALESCE(l.verdict,''))='APPROVED' OR upper(COALESCE(l.publication_review_status,''))='APPROVED'
        THEN 'APPROVED' ELSE 'PENDING_VERIFICATION' END,
      'publication_lane','QNSA_ZENITH_REVIEWED_V1','normalization_run_complete',true,
      'raw_lineage_verified',true,'identity_reconciliation_status','RELEASE_SAFE_EXACT_SOURCE_REFERENCE',
    ...
    FROM selected s JOIN staging.listings l ON l.id=s.id
  ...
  SELECT jsonb_build_object(
    'rows',COALESCE((SELECT jsonb_agg(row_data ORDER BY has_image DESC,has_price DESC,sort_reference,sort_id) FROM contracted),'[]'::jsonb),
...
REVOKE ALL ON FUNCTION public.qnsa_zenith_ordered_candidate_page(INTEGER,INTEGER,TEXT)
  FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.qnsa_zenith_ordered_candidate_page(INTEGER,INTEGER,TEXT)
...
SET search_path=public,staging,pg_catalog AS $$
  SELECT CASE WHEN p_brand='Zenith' THEN public.qnsa_zenith_ordered_candidate_page(
