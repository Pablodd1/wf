...
// BROWSER_BIN env override / Linux chromium detection (Phase 10 test-infra).
const BROWSER_BIN = process.env.BROWSER_BIN
  || (fs.existsSync(EDGE_PATH) ? EDGE_PATH : null)
  || (fs.existsSync(CHROME_PATH) ? CHROME_PATH : null)
  || (fs.existsSync('/usr/bin/chromium') ? '/usr/bin/chromium' : CHROME_PATH);
