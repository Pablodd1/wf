-- Keep ordinary scalar-brand paging on v6. For image-only requests, begin
-- with the small immutable verified-image bridge rather than filtering the
-- entire current-listing stream for rare customer-safe images.

CREATE OR REPLACE FUNCTION public.curated_luxury_shadow_customer_page_keys_v7(
  p_run_id uuid,
  p_brand text,
...
BEGIN
  IF NOT p_images_only THEN
    RETURN public.curated_luxury_shadow_customer_page_keys_v6(
      p_run_id,p_brand,p_intents,p_countries,p_priced_only,false,p_search,
      p_reference_key,p_listing_lane,p_after_lane,p_after_timestamp,p_after_key,
      p_after_timestamp_is_null,p_limit);
  END IF;

  RETURN (
    WITH verified_images AS MATERIALIZED (
      SELECT DISTINCT l.current_listing_key,l.raw_occurrence_key
      ...
      WHERE l.run_id=p_run_id
        AND l.image_evidence_type='SELLER_LISTING_IMAGE'
        AND a.customer_safe
    ), single_candidates AS MATERIALIZED (
      SELECT c.current_listing_key,c.source_timestamp,0::smallint listing_lane
      FROM verified_images image
      JOIN public.curated_luxury_current_listings_shadow c
        ON c.run_id=p_run_id AND c.current_listing_key=image.current_listing_key
       AND c.latest_raw_occurrence_key=image.raw_occurrence_key
      WHERE c.brand=p_brand AND p_brand IN ('Rolex','Patek Philippe')
      ...
      SELECT * FROM single_candidates UNION ALL SELECT * FROM multi_candidates
...
REVOKE ALL ON FUNCTION public.curated_luxury_shadow_customer_page_keys_v7(
  uuid,text,text[],text[],boolean,boolean,text,text,smallint,smallint,
  timestamptz,text,boolean,integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.curated_luxury_shadow_customer_page_keys_v7(
  uuid,text,text[],text[],boolean,boolean,text,text,smallint,smallint,
