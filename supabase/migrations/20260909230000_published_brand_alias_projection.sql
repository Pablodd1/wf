-- Exact, reviewed source aliases already present in dictionaries/brands.json,
-- api/_lib/catalog.js and api/pipeline-parse.js. No guessing from references.
-- A presentation/filter projection only: frozen payloads and source hashes stay intact.
BEGIN;
SET LOCAL lock_timeout='5s';
CREATE OR REPLACE FUNCTION wf_canonical_staging.published_browse_brand_v1(p_brand text)
RETURNS text LANGUAGE sql IMMUTABLE STRICT PARALLEL SAFE SET search_path='' AS $alias$
 SELECT COALESCE('{"datejust":"Rolex","day-date":"Rolex","gmt-master ii":"Rolex","a. lange & sohne":"A. Lange & Söhne","f.p.journe":"F.P. Journe","jacob & co.":"Jacob & Co","h. moser & cie.":"H. Moser & Cie","moser":"H. Moser & Cie","moser & cie.":"H. Moser & Cie","bulgari":"Bvlgari","tag heuer":"TAG Heuer"}'::jsonb->>lower(btrim(p_brand)),btrim(p_brand));
$alias$;
REVOKE ALL ON FUNCTION wf_canonical_staging.published_browse_brand_v1(text) FROM PUBLIC,anon,authenticated,service_role;

DO $filters$
DECLARE item record; definition text; updated text; changed integer=0;
 pattern text=$pattern$lower\((m\.payload[[:space:]]*->>[[:space:]]*'brand'|member\.payload[[:space:]]*->>[[:space:]]*'brand'|v\.brand)\)[[:space:]]*=[[:space:]]*lower\(p_brand\)$pattern$;
BEGIN
 FOR item IN
  ...
  WHERE (n.nspname='public' AND p.proname=ANY(ARRAY[
   ...
   'get_price_research_snapshot_stats','get_price_research_canary_keyset_v4','get_price_research_wtb_demand_v3',
   'get_price_research_snapshot_membership']))
 ...
 LOOP
  ...
  updated=regexp_replace(definition,pattern,
   $replacement$lower(wf_canonical_staging.published_browse_brand_v1(\1)) = lower(wf_canonical_staging.published_browse_brand_v1(p_brand))$replacement$,'g');
  IF updated=definition THEN RAISE EXCEPTION 'brand_alias_filter_definition_mismatch: %',item.proname; END IF;
  updated=replace(updated,$search$lower(COALESCE(m.payload ->> 'brand', ''))$search$,
    $search$lower(COALESCE(wf_canonical_staging.published_browse_brand_v1(m.payload ->> 'brand'), ''))$search$);
  EXECUTE updated;changed=changed+1;
 END LOOP;
 IF changed<>11 THEN RAISE EXCEPTION 'brand_alias_filter_function_count_mismatch'; END IF;
END $filters$;
...
BEGIN
  ...
  WITH raw_entries AS MATERIALIZED (
    SELECT wf_canonical_staging.published_browse_brand_v1(brand) brand,model,reference,listing_count,wts_count,wtb_count,image_url FROM jsonb_to_recordset(v_entries) AS e(brand text,model text,reference text,listing_count int,wts_count int,wtb_count int,image_url text)
  ), labels AS (
    ...
    FROM entries WHERE p_brand IS NOT NULL AND lower(brand)=lower(wf_canonical_staging.published_browse_brand_v1(p_brand)) GROUP BY model
  ), refs AS (
    SELECT reference,model,listing_count,wts_count,wtb_count,image_url FROM entries
    WHERE p_brand IS NOT NULL AND lower(brand)=lower(wf_canonical_staging.published_browse_brand_v1(p_brand)) AND reference IS NOT NULL
