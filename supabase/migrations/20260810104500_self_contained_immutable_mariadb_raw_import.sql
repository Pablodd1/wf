-- Forward-only, self-contained immutable MariaDB raw-import contract.
--
-- The production pipeline project did not contain public.raw_messages, which
-- was previously defined by an unrelated historical migration. This migration
-- creates only the raw evidence envelope, its immutable versions/checkpoints,
-- and the two import RPCs. It never normalizes or publishes watch_records.
...
REVOKE ALL ON public.raw_messages FROM PUBLIC, anon, authenticated;
...
COMMENT ON TABLE public.raw_messages IS
  'Immutable source envelopes copied from MariaDB. Customer roles cannot read this evidence table.';
