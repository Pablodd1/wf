...
- Independent official ECB historical-rate resolution keyed by immutable `source_created_on`; stored conversion source/rate/timestamp are comparison evidence only.
...
The workflow is manual, canonical-QNSA-pinned, bound to the GitHub `Production` environment, migration-hash pinned, and atomically verifies private access plus unchanged customer surfaces, publication controls, staging row versions, and raw-table row versions. GitHub currently reports no reviewer protection rules for that environment; the exact confirmation string and stable run key are therefore the operative dispatch gates. The workflow must be reviewed before it can exist on the default branch and be manually dispatched.
