-- Exact, reversible, null-only completion for canonical Patek Philippe listings.
-- Reuses the private null-only ledger while preserving all populated values and immutable raw evidence.

BEGIN;

CREATE OR REPLACE FUNCTION public.begin_qnsa_patek_null_only_completion(
...
RETURNS jsonb LANGUAGE sql SECURITY DEFINER SET search_path=public,pg_catalog AS $$
  SELECT public.begin_qnsa_rolex_null_only_completion(p_run_key,p_mode,p_manifest_sha256,p_expected_count);
$$;

CREATE OR REPLACE FUNCTION public.stage_qnsa_patek_null_only_completion(p_run_key text,p_records jsonb)
...
BEGIN
  ...
  FOR r IN SELECT value FROM jsonb_array_elements(p_records) LOOP
    ...
    IF NOT FOUND OR l.brand_normalized IS DISTINCT FROM 'Patek Philippe'
      ...
      RAISE EXCEPTION 'Exact Patek lineage mismatch for %',r->>'listing_id'; END IF;
...
CREATE OR REPLACE FUNCTION public.finalize_qnsa_patek_null_only_completion(p_run_key text)
RETURNS jsonb LANGUAGE sql SECURITY DEFINER SET search_path=public,pg_catalog AS $$
  SELECT public.finalize_qnsa_rolex_null_only_completion(p_run_key);
$$;

CREATE OR REPLACE FUNCTION public.activate_qnsa_patek_null_only_completion(p_run_key text)
...
BEGIN
  ...
  LEFT JOIN staging.listings l ON l.id=p.listing_id AND l.brand_normalized='Patek Philippe'
...
CREATE OR REPLACE FUNCTION public.rollback_qnsa_patek_null_only_completion(p_run_key text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,staging,pg_catalog SET statement_timeout='180s' AS $$
...
REVOKE ALL ON FUNCTION public.begin_qnsa_patek_null_only_completion(text,text,text,integer),
  public.stage_qnsa_patek_null_only_completion(text,jsonb),public.finalize_qnsa_patek_null_only_completion(text),
  public.activate_qnsa_patek_null_only_completion(text),public.rollback_qnsa_patek_null_only_completion(text)
  FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.begin_qnsa_patek_null_only_completion(text,text,text,integer),
  public.stage_qnsa_patek_null_only_completion(text,jsonb),public.finalize_qnsa_patek_null_only_completion(text),
  public.activate_qnsa_patek_null_only_completion(text),public.rollback_qnsa_patek_null_only_completion(text)
  TO service_role;
