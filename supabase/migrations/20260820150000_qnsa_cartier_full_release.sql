-- Exact, reversible publication control for the reconciled Cartier cohort.
-- Source rows remain immutable in staging.listings.

CREATE TABLE IF NOT EXISTS public.qnsa_cartier_release_runs (
...
CREATE TABLE IF NOT EXISTS public.qnsa_cartier_release_manifest (
  listing_id uuid PRIMARY KEY,
  release_run_key text NOT NULL REFERENCES public.qnsa_cartier_release_runs(release_run_key),
  ...
  identity_source text NOT NULL CHECK (identity_source IN ('CATALOG_CARTIER_REFERENCE','SOURCE_CARTIER_IDENTITY')),
...
CREATE TABLE IF NOT EXISTS public.qnsa_cartier_release_control (
  singleton boolean PRIMARY KEY DEFAULT true CHECK (singleton),
  enabled boolean NOT NULL DEFAULT false,
  release_run_key text REFERENCES public.qnsa_cartier_release_runs(release_run_key),
...
INSERT INTO public.qnsa_cartier_release_control(singleton, enabled, release_mode)
VALUES (true, false, 'DISABLED')
ON CONFLICT (singleton) DO NOTHING;

ALTER TABLE public.qnsa_cartier_release_runs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.qnsa_cartier_release_manifest ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.qnsa_cartier_release_control ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON public.qnsa_cartier_release_runs,
  public.qnsa_cartier_release_manifest,
  public.qnsa_cartier_release_control FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.qnsa_cartier_release_runs,
  public.qnsa_cartier_release_manifest,
  public.qnsa_cartier_release_control TO service_role;

CREATE INDEX IF NOT EXISTS idx_qnsa_cartier_manifest_run_order
ON public.qnsa_cartier_release_manifest(release_run_key, release_order);

CREATE OR REPLACE FUNCTION public.qnsa_cartier_release_count(
...
AS $$
  ...
  JOIN public.qnsa_cartier_release_manifest m
...
CREATE OR REPLACE FUNCTION public.qnsa_cartier_reference_rows(
  p_reference text,
...
AS $$
  SELECT row_data
  FROM public.qnsa_cartier_page_rows(p_limit, p_offset, p_listing_type, p_reference);
$$;
...
REVOKE ALL ON FUNCTION public.qnsa_cartier_release_count(text),
  ...
  public.qnsa_cartier_reference_rows(text,integer,integer,text),
  public.qnsa_cartier_reference_index()
