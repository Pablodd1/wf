...
const RELEASE_RUN_KEY = 'tudor-20260820-v1';
const EXPECTED_COUNT = 2555;
const EXPECTED_PLAN_SHA256 = 'e3b20565929f9f7bda5dc0f4f8ed24027753f35321b4139295748aa85434efa2';
const MIGRATIONS = [
  '20260820190000_qnsa_tudor_release.sql',
  '20260820191000_qnsa_controlled_model_browse_tudor.sql',
].map(file => path.join(__dirname, '..', '..', 'supabase', 'migrations', file));
...
function catalogReferenceLookups() {
  ...
  const exact = entries.filter(entry => /^[A-Z0-9]{4,50}$/.test(entry.key) && entry.model);
  const familyModels = new Map();
  for (const entry of entries) {
    const familyKey = entry.reference.toUpperCase().replace(/^M/, '').replace(/-[0-9]{4}$/, '').replace(/[^A-Z0-9]/g, '');
    if (!/^(?=.*[0-9])[A-Z0-9]{4,30}$/.test(familyKey)) continue;
    if (!familyModels.has(familyKey)) familyModels.set(familyKey, new Set());
    familyModels.get(familyKey).add(entry.model);
  }
  const family = [...familyModels.entries()]
    .filter(([, models]) => models.size === 1 && [...models][0])
    .map(([key, models]) => ({ key, model: [...models][0] }));
  if (exact.length < 150 || family.length < 20) throw new Error('Tudor catalog reference set is incomplete.');
  return { exact, family };
}
...
WITH catalog_exact(reference_key, model) AS (
  ...
  SELECT n.*,
    COALESCE(n.raw_message_text, '') ~* '(^|[^[:alnum:]])tudor([^[:alnum:]]|$)' AS raw_tudor_brand,
    n.raw_collection_model IS NOT NULL AS raw_tudor_collection,
    ...
    n.reference_key ~ '[0-9]' AND length(n.reference_key) BETWEEN 4 AND 30
  ...
  WHERE source_intent = 'WTS' AND catalog_tudor_reference
...
SELECT id::text AS listing_id, source_hash, source_candidate_hash, release_order,
  NULLIF(btrim(reference_normalized), '') AS public_reference,
  COALESCE(NULLIF(btrim(resolved_model), ''), 'Reference-only listings') AS public_model,
  CASE WHEN catalog_tudor_reference THEN 'CATALOG_TUDOR_REFERENCE'
    ELSE 'SOURCE_TUDOR_IDENTITY' END AS identity_source,
  catalog_tudor_reference,
  ...
  END AS price_lane,
  (raw_tudor_brand OR raw_tudor_collection) AS source_tudor_identity,
...
async function loadPlan() {
  const rows = await managementQuery(candidateSql(catalogReferenceLookups()), true);
  const plan = (rows || []).map(row => ({
    ...
    public_model: String(row.public_model || 'Reference-only listings'),
    identity_source: String(row.identity_source),
    catalog_reference_confirmed: row.catalog_tudor_reference === true,
    listing_type: String(row.listing_type),
    price_lane: String(row.price_lane),
    source_tudor_identity: row.source_tudor_identity === true,
...
function selectCanary(plan) {
  const predicates = [
    ...
    row => row.identity_source === 'SOURCE_TUDOR_IDENTITY',
...
async function applySchema() {
  const migration = MIGRATIONS.map(file => fs.readFileSync(file, 'utf8')
    .replace(/^\s*BEGIN;\s*$/gmi, '').replace(/^\s*COMMIT;\s*$/gmi, '')).join('\n');
...
async function ensureRun(plan, planSha) {
  ...
  await managementQuery(`BEGIN;
    INSERT INTO public.qnsa_tudor_release_runs(
    ...
    ON CONFLICT (release_run_key) DO UPDATE SET updated_at = now()
      WHERE qnsa_tudor_release_runs.plan_sha256 = EXCLUDED.plan_sha256
        AND qnsa_tudor_release_runs.planned_count = EXCLUDED.planned_count
        AND qnsa_tudor_release_runs.release_mode <> 'ROLLED_BACK';
    DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM public.qnsa_tudor_release_runs
...
async function insertBatch(batch) {
  ...
  const rows = await managementQuery(`BEGIN;
    ...
    INSERT INTO public.qnsa_tudor_release_manifest(
    ...
    SELECT count(*)::integer AS reconciled FROM public.qnsa_tudor_release_manifest
...
async function activate(mode, expectedCount, planSha) {
  const rows = await managementQuery(`BEGIN;
    UPDATE public.qnsa_tudor_release_control SET enabled=true,
    ...
    UPDATE public.qnsa_tudor_release_runs SET release_mode=${sqlText(mode)}, updated_at=now()
    ...
    FROM public.qnsa_tudor_release_manifest
    WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
  COMMIT;`);
...
async function rollback() {
  const rows = await managementQuery(`BEGIN;
    UPDATE public.qnsa_tudor_release_control SET enabled=false, release_run_key=NULL,
    ...
    DELETE FROM public.qnsa_tudor_release_manifest WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
    UPDATE public.qnsa_tudor_release_runs SET release_mode='ROLLED_BACK', updated_at=now()
      WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
    SELECT count(*)::integer AS remaining FROM public.qnsa_tudor_release_manifest
      WHERE release_run_key=${sqlText(RELEASE_RUN_KEY)};
  COMMIT;`);
...
async function main() {
  ...
  const output = path.resolve(arg('output', path.join('audit-output', 'tudor-release-report.json')));
  ...
  const confirmation = String(process.env.TUDOR_RELEASE_CONFIRMATION || '');
  const required = mode === 'audit' ? 'AUDIT_QNSA_TUDOR_RELEASE'
    : mode === 'canary' ? 'APPLY_QNSA_TUDOR_CANARY'
    : mode === 'full' ? 'APPLY_QNSA_TUDOR_FULL'
    : 'ROLLBACK_QNSA_TUDOR_RELEASE';
  ...
  if (mode === 'rollback') {
    ...
    planSummary = {
      source_expected: 3934,
      source_found: 3934,
      identity_held: 506,
      unique_individual_listings: plan.length,
      duplicates_excluded: 873,
