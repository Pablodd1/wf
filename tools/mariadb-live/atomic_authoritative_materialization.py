...
print("=== Blue-Green Atomic Authoritative Materialization ===")
...
print("Step 4: Executing atomic blue-green table swap inside single transaction...")
cur.execute("""
  BEGIN;
...
print("Step 5: Verifying active table after atomic swap...")
...
assert tot_active == EXPECTED_ROWS, f"Active table count mismatch: {tot_active}"
assert uniq_active == EXPECTED_ROWS, f"Active table unique ID mismatch: {uniq_active}"
print("ATOMIC_SWAP_SUCCESS: Active table is fully populated and verified with zero downtime!")
