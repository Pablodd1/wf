...
CREATE TABLE price_research_shadow.observations (
  ...
  CHECK (source_currency IN ('USD','USDT') OR verified_usd_amount IS NULL OR
    (fx_provider IS NOT NULL AND fx_rate > 0 AND fx_effective_date IS NOT NULL))
);
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'conversion_source',l.conversion_source,'price_research_status',l.price_research_status,
...
/* Retained in repository history only; deliberately not installed. The initial
   broad reconciliation draft was superseded before release by the bounded
   exact-reference materializer below.
...
AS $function$
DECLARE v_source_run_key text; v_expected bigint; v_actual bigint; v_result_sha text;
BEGIN
  PERFORM pg_advisory_xact_lock(hashtext('phase7b_verified_price_shadow'));
  SELECT source_run_key,source_observation_count INTO v_source_run_key,v_expected
  FROM price_research_shadow.runs WHERE run_key=p_run_key AND status='RUNNING' FOR UPDATE;
  IF v_source_run_key IS NULL THEN RAISE EXCEPTION 'Phase 7B run is not running'; END IF;
  SELECT count(*) INTO v_actual FROM price_research_shadow.observations WHERE run_key=p_run_key;
  IF v_actual<>v_expected THEN RAISE EXCEPTION 'Processed % observations but expected %',v_actual,v_expected; END IF;
  UPDATE price_research_shadow.runs SET status='RECONCILING',updated_at=now() WHERE run_key=p_run_key;

  -- Total Listings uses the same explicit single-watch publication contract
  -- as qnsa_market_feed_page_rows and never combines staging/release/PR counts.
  WITH catalog AS (
    SELECT * FROM price_research_shadow.catalog_references WHERE run_key=p_run_key
  ), published AS MATERIALIZED (
    SELECT l.*,c.canonical_model,c.canonical_reference
    FROM staging.listings l JOIN catalog c ON c.brand=l.brand_normalized
      AND c.canonical_reference=l.reference_normalized
    WHERE l.normalization_run_key=v_source_run_key
    ...
    SELECT brand_normalized brand,canonical_model,canonical_reference,
      count(DISTINCT id) total_published_listings,
      count(DISTINCT id) FILTER(WHERE upper(COALESCE(listing_type,intent,''))='WTS') wts_listings,
      count(DISTINCT id) FILTER(WHERE upper(COALESCE(listing_type,intent,''))='WTB') wtb_listings,
      count(DISTINCT id) FILTER(WHERE COALESCE(price_usd,price_normalized,0)>0) priced_listings,
      count(DISTINCT id) FILTER(WHERE btrim(COALESCE(image_url,source_media_url_candidate,''))~*'^https?://[^[:space:]]+$') image_linked_listings
    FROM published GROUP BY brand_normalized,canonical_model,canonical_reference
  ), class_counts AS (
    SELECT brand,canonical_reference,count(*) legacy_pr_observations,
      count(*) FILTER(WHERE price_evidence_classification='VERIFIED_IN_NEW_COHORT') verified_pr_observations,
      count(*) FILTER(WHERE price_evidence_classification IN
        ('LEGACY_USD_DEFAULTED','BARE_DOLLAR_AMBIGUOUS','CURRENCYLESS_AMOUNT','CURRENCYLESS_KM',
         ...
         'SOURCE_PRICE_CONFLICT','REFERENCE_AMBIGUOUS','REVIEW_REQUIRED')) review_required_observations,
      count(*) FILTER(WHERE price_evidence_classification IN
        ('REFERENCE_INVALID','SOURCE_NOT_RECONCILABLE','OTHER')) excluded_observations
    FROM price_research_shadow.observations WHERE run_key=p_run_key
    GROUP BY brand,canonical_reference
  ), current_values AS (
    SELECT brand,canonical_reference,current_usd_amount amount
    FROM price_research_shadow.observations WHERE run_key=p_run_key AND canonical_reference IS NOT NULL
  ), current_bounds AS (
    SELECT brand,canonical_reference,count(*) raw_count,
      percentile_cont(.25) WITHIN GROUP(ORDER BY amount) q1,
      percentile_cont(.75) WITHIN GROUP(ORDER BY amount) q3
    FROM current_values GROUP BY brand,canonical_reference
  ), current_clean AS (
    SELECT v.* FROM current_values v JOIN current_bounds b USING(brand,canonical_reference)
    WHERE v.amount BETWEEN b.q1-3.0*(b.q3-b.q1) AND b.q3+3.0*(b.q3-b.q1)
  ), current_stats AS (
    SELECT b.brand,b.canonical_reference,b.raw_count,count(c.amount) clean_count,
      percentile_cont(.5) WITHIN GROUP(ORDER BY c.amount) median,avg(c.amount) mean,
      min(c.amount) min,max(c.amount) max
    FROM current_bounds b LEFT JOIN current_clean c USING(brand,canonical_reference)
    GROUP BY b.brand,b.canonical_reference,b.raw_count
  ), verified_values AS (
    SELECT brand,canonical_reference,verified_usd_amount amount
    FROM price_research_shadow.observations WHERE run_key=p_run_key
      AND price_evidence_classification='VERIFIED_IN_NEW_COHORT'
  ), verified_bounds AS (
    SELECT brand,canonical_reference,count(*) raw_count,
      percentile_cont(.25) WITHIN GROUP(ORDER BY amount) q1,
      percentile_cont(.75) WITHIN GROUP(ORDER BY amount) q3
    FROM verified_values GROUP BY brand,canonical_reference
  ), verified_clean AS (
    SELECT v.* FROM verified_values v JOIN verified_bounds b USING(brand,canonical_reference)
    WHERE v.amount BETWEEN b.q1-3.0*(b.q3-b.q1) AND b.q3+3.0*(b.q3-b.q1)
  ), verified_stats AS (
    SELECT b.brand,b.canonical_reference,b.raw_count,count(c.amount) clean_count,
      percentile_cont(.5) WITHIN GROUP(ORDER BY c.amount) median,avg(c.amount) mean,
      min(c.amount) min,max(c.amount) max
    FROM verified_bounds b LEFT JOIN verified_clean c USING(brand,canonical_reference)
    GROUP BY b.brand,b.canonical_reference,b.raw_count
  ), result AS (
    SELECT c.brand,c.canonical_model,c.canonical_reference,
      'QNSA_GENERAL_MARKET_FEED_V1_SINGLE_WATCH_WTS_WTB'::text publication_contract,
      COALESCE(i.total_published_listings,0) total_published_listings,
      COALESCE(i.wts_listings,0) wts_listings,COALESCE(i.wtb_listings,0) wtb_listings,
      COALESCE(i.priced_listings,0) priced_listings,COALESCE(i.image_linked_listings,0) image_linked_listings,
      COALESCE(cc.legacy_pr_observations,0) legacy_pr_observations,
      COALESCE(cc.verified_pr_observations,0) verified_pr_observations,
      COALESCE(cc.review_required_observations,0) review_required_observations,
      COALESCE(cc.excluded_observations,0) excluded_observations,
      COALESCE(cs.raw_count,0) current_observation_count,COALESCE(cs.clean_count,0) current_qualified_comparable_count,
      cs.median current_median,cs.mean current_mean,cs.min current_min,cs.max current_max,
      COALESCE(cs.raw_count,0)>=2 current_analytics_ready,
      COALESCE(vs.raw_count,0) verified_observation_count,COALESCE(vs.clean_count,0) verified_qualified_comparable_count,
      vs.median verified_median,vs.mean verified_mean,vs.min verified_min,vs.max verified_max,
      COALESCE(vs.raw_count,0)>=2 verified_analytics_ready
    FROM catalog c LEFT JOIN inventory i USING(brand,canonical_model,canonical_reference)
    LEFT JOIN class_counts cc USING(brand,canonical_reference)
    LEFT JOIN current_stats cs USING(brand,canonical_reference)
    LEFT JOIN verified_stats vs USING(brand,canonical_reference)
  )
  ...
  SELECT p_run_key,r.*,
    encode(extensions.digest(convert_to(to_jsonb(r)::text,'UTF8'),'sha256'),'hex') FROM result r;

  -- Exact-reference + dial benchmarks mirror the product's 3.0x IQR and
  -- minimum-two-comparables rating contract, but remain private and inactive.
  WITH source AS (
    SELECT o.listing_id,o.brand,o.canonical_reference,o.current_usd_amount,
      o.verified_usd_amount,o.price_evidence_classification,
    ...
    WHERE o.run_key=p_run_key AND o.canonical_reference IS NOT NULL
  ), current_bounds AS (
    SELECT brand,canonical_reference,dial_key,count(*) n,
    ...
    FROM source GROUP BY brand,canonical_reference,dial_key
  ), current_stats AS (
    SELECT s.brand,s.canonical_reference,s.dial_key,b.n,count(*) clean_n,
      percentile_cont(.5) WITHIN GROUP(ORDER BY s.current_usd_amount) median,
      min(s.current_usd_amount) min,max(s.current_usd_amount) max,
      bool_or(s.price_evidence_classification<>'VERIFIED_IN_NEW_COHORT') has_untrusted
    FROM source s JOIN current_bounds b USING(brand,canonical_reference,dial_key)
    WHERE s.current_usd_amount BETWEEN b.q1-3.0*(b.q3-b.q1) AND b.q3+3.0*(b.q3-b.q1)
    GROUP BY s.brand,s.canonical_reference,s.dial_key,b.n
  ), verified_bounds AS (
    SELECT brand,canonical_reference,dial_key,count(*) n,
      percentile_cont(.25) WITHIN GROUP(ORDER BY verified_usd_amount) q1,
      percentile_cont(.75) WITHIN GROUP(ORDER BY verified_usd_amount) q3
    FROM source WHERE price_evidence_classification='VERIFIED_IN_NEW_COHORT'
    GROUP BY brand,canonical_reference,dial_key
  ), verified_stats AS (
    SELECT s.brand,s.canonical_reference,s.dial_key,b.n,count(*) clean_n,
      percentile_cont(.5) WITHIN GROUP(ORDER BY s.verified_usd_amount) median,
      min(s.verified_usd_amount) min,max(s.verified_usd_amount) max
    FROM source s JOIN verified_bounds b USING(brand,canonical_reference,dial_key)
    WHERE s.price_evidence_classification='VERIFIED_IN_NEW_COHORT'
      AND s.verified_usd_amount BETWEEN b.q1-3.0*(b.q3-b.q1) AND b.q3+3.0*(b.q3-b.q1)
    GROUP BY s.brand,s.canonical_reference,s.dial_key,b.n
  ), ratings AS (
    SELECT s.listing_id,s.brand,s.canonical_reference,s.dial_key,s.current_usd_amount listing_price_usd,
      COALESCE(cs.clean_n,0) current_comparable_count,COALESCE(vs.clean_n,0) verified_comparable_count,
      CASE WHEN COALESCE(cs.clean_n,0)<2 OR s.current_usd_amount NOT BETWEEN cs.min AND cs.max THEN 'NOT_RATED'
        WHEN s.current_usd_amount<=cs.median*.95 THEN 'GOOD'
        WHEN s.current_usd_amount<=cs.median*1.05 THEN 'MARKET' ELSE 'HIGH' END current_rating,
      CASE WHEN COALESCE(vs.clean_n,0)<2 OR s.current_usd_amount NOT BETWEEN vs.min AND vs.max THEN 'NOT_RATED'
        WHEN s.current_usd_amount<=vs.median*.95 THEN 'GOOD'
        WHEN s.current_usd_amount<=vs.median*1.05 THEN 'MARKET' ELSE 'HIGH' END verified_rating,
      COALESCE(cs.has_untrusted,false) has_untrusted
    FROM source s LEFT JOIN current_stats cs USING(brand,canonical_reference,dial_key)
    LEFT JOIN verified_stats vs USING(brand,canonical_reference,dial_key)
  ), result AS (
    SELECT r.*,
      CASE WHEN current_rating<>'NOT_RATED' AND verified_rating='NOT_RATED' THEN 'OPEN_FOR_RATING'
        ...
        ELSE 'UNCHANGED' END impact_class FROM ratings r
  )
  ...
  SELECT p_run_key,r.listing_id,r.brand,r.canonical_reference,r.dial_key,r.listing_price_usd,
    r.current_comparable_count,r.verified_comparable_count,r.current_rating,r.verified_rating,r.impact_class,
    encode(extensions.digest(convert_to(to_jsonb(r)::text,'UTF8'),'sha256'),'hex') FROM result r;

  SELECT encode(extensions.digest(convert_to(string_agg(census_sha256,'' ORDER BY brand,canonical_reference),'UTF8'),'sha256'),'hex')
    INTO v_result_sha FROM price_research_shadow.reference_census WHERE run_key=p_run_key;
  UPDATE price_research_shadow.runs SET status='COMPLETE',processed_observation_count=v_actual,
    verified_observation_count=(SELECT count(*) FROM price_research_shadow.observations
      WHERE run_key=p_run_key AND price_evidence_classification='VERIFIED_IN_NEW_COHORT'),
    result_sha256=v_result_sha,updated_at=now(),completed_at=now() WHERE run_key=p_run_key;
  RETURN jsonb_build_object('run_key',p_run_key,'status','COMPLETE','observations',v_actual,
    'verified',(SELECT verified_observation_count FROM price_research_shadow.runs WHERE run_key=p_run_key),
    'references',(SELECT count(*) FROM price_research_shadow.reference_census WHERE run_key=p_run_key),
