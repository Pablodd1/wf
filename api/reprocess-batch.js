    ...
    for (const record of records || []) {
      ...
      try {
        ...
        if (parsed.brand || parsed.ref || parsed.price) {
          // P0 FIX: Convert price to USD using parser's toUSD function
          // This prevents storing HKD/EUR prices as USD (e.g., 1,080,000 HKD → $138,240 USD, not $1,080,000)
          const priceUSD = parser.toUSD(parsed.price, parsed.currency);
          const { error: updateError } = await supabase.from('watch_records').update({
            ...
            price_usd: priceUSD || parsed.price || null, currency: parsed.currency || 'USD', confidence: parsed.confidence || 0,
