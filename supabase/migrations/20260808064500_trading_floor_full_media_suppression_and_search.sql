...
-- Migration ID: 20260808064500_trading_floor_full_media_suppression_and_search.sql
-- Description:
-- 1. Complete child media suppression for parent_id IS NOT NULL
-- 2. Upper case reference_search_key
-- 3. Exclude unresolved bundle parents and other strict filters
-- ============================================================================
...
SELECT id,
    ...
    CASE WHEN parent_id IS NOT NULL THEN NULL ELSE front_image END AS front_image,
    CASE 
    ...
    CASE 
        WHEN parent_id IS NULL 
             AND is_bundle = false 
             AND (image_url LIKE 'http%' OR front_image LIKE 'http%') 
             AND COALESCE(source_image_preserved, true) = true 
             AND COALESCE(image_url_resolvable, true) = true 
        THEN true 
        ELSE false 
    END AS has_exact_source_image,
    image_provenance,
    COALESCE(source_image_preserved, (front_image LIKE 'http%' OR image_url LIKE 'http%')) AS source_image_preserved,
    CASE WHEN parent_id IS NOT NULL THEN false ELSE COALESCE(image_url_resolvable, (image_url LIKE 'http%' OR front_image LIKE 'http%')) END AS image_url_resolvable,
    CASE WHEN parent_id IS NOT NULL THEN false ELSE COALESCE(visually_verified, false) END AS visually_verified,
    CASE WHEN parent_id IS NOT NULL THEN NULL ELSE storage_key END AS storage_key,
    CASE WHEN parent_id IS NOT NULL THEN '[]'::jsonb ELSE attachment_keys END AS attachment_keys,
    CASE WHEN parent_id IS NOT NULL THEN NULL ELSE mime_type END AS mime_type,
    media_fingerprint,
    ...
    upper(regexp_replace(reference_normalized::text, '[^a-zA-Z0-9]'::text, ''::text, 'g'::text)) AS reference_search_key,
...
CREATE INDEX IF NOT EXISTS idx_trading_floor_ordering_20260808064500 
ON staging.listings (
    (CASE 
        WHEN parent_id IS NULL 
             AND is_bundle = false 
