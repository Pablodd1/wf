...
async function main() {
  ...
  try {
    ...
    const memberCount = Number((await db.query('select count(*) as n from wf_canonical_staging.keyset_snapshot_members')).rows[0].n);
    for (let i = 0; i < 100; i++) {
      assert.equal(await snapshot('trading_floor'), tf);
      assert.equal(await snapshot('price_research'), pr);
    }
    assert.equal(Number((await db.query('select count(*) as n from wf_canonical_staging.keyset_snapshot_members')).rows[0].n), memberCount);
    report.assertions.push({ name: '200 snapshot opens reuse two immutable publication copies', result: 'PASS', member_rows: memberCount });
    const analytics = async () => ({
      facets: (await db.query('select * from public.get_price_research_snapshot_facets($1,null)', [pr])).rows,
      breakdown: (await db.query('select * from public.get_price_research_snapshot_breakdown($1,null)', [tf])).rows,
      stats: (await db.query("select * from public.get_price_research_snapshot_stats($1,'Patek Philippe','7128/1G',null,'Blue','New')", [pr])).rows,
      peer: (await db.query('select public.get_price_research_demand_snapshot($1) as id', [pr])).rows[0].id,
    });
    const frozenAnalytics = await analytics();
    assert.equal(frozenAnalytics.peer, tf);
    assert.equal(frozenAnalytics.stats.length, 1);
    assert.equal(Number(frozenAnalytics.stats[0].qualified_count), 3);
    const budget = 'a'.repeat(64);
    const decisions = (await Promise.all([db, writer].map(async connection => {
      const results = [];
      for (let i = 0; i < 50; i++) results.push(await connection.query('select public.consume_listing_contact_budget($1) as allowed', [budget]));
      return results;
    }))).flat();
    assert.equal(decisions.filter(result => result.rows[0].allowed).length, 30);
    report.assertions.push({ name: '100 concurrent attempts across two database sessions admit exactly 30', result: 'PASS' });
    ...
    assert.deepEqual(await analytics(), frozenAnalytics);
    report.assertions.push({ name: 'statistics, facets, breakdown and demand pair stay frozen after separate-session mutations', result: 'PASS' });
