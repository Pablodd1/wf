-- Forward-only Trading Floor publication contract.
--
-- Trading Floor and Price Research have intentionally different admission
-- policies. This view admits:
--   1. existing reviewed, APPROVED single listings at >= 90 confidence; and
--   2. single WTS/WTB rows from a fully reconciled QNSA normalized-staging run,
--      clearly labeled PENDING_VERIFICATION.
--
-- It never admits bundle parents, bundle children, exact duplicates, rejected
-- rows, incomplete normalization runs, or unreviewed images. Price Research is
...
COMMENT ON VIEW public.reviewed_workbook_market_source_v2 IS
  'Trading Floor singles: reviewed approvals plus completed QNSA normalized-staging rows labeled pending verification. Bundles, unreviewed media, and analytics-ineligible state remain excluded.';
