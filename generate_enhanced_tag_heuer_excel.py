...
print("Querying TAG Heuer records in chronological order for duplicate tracking...", flush=True)
cur.execute("""
...
print(f"Retrieved {len(rows)} TAG Heuer records! Normalizing with WatchFacts Engine & Tracking Duplicates...", flush=True)
...
seen_reposts = {}    # (seller, ref, price) -> count
total_duplicates = 0
...
for idx, r in enumerate(rows):
    ...
    # Construct complete resolvable URL
    if raw_img:
    ...
    if model_norm in ("Unspecified", "None", ""):
        if 'monaco' in text_clean: model_norm = "Monaco"
        elif 'carrera' in text_clean: model_norm = "Carrera"
        elif 'aquaracer' in text_clean: model_norm = "Aquaracer"
        elif 'formula 1' in text_clean or 'formula1' in text_clean or 'f1' in text_clean: model_norm = "Formula 1"
        elif 'autavia' in text_clean: model_norm = "Autavia"
        elif 'monza' in text_clean: model_norm = "Monza"
        elif 'link' in text_clean: model_norm = "Link"
        elif 'connected' in text_clean: model_norm = "Connected"
        elif 'super 2000' in text_clean or '2000' in text_clean: model_norm = "2000 Series"
...
# Reverse back to latest first for presentation
normalized_records.reverse()
for idx, r in enumerate(normalized_records, 1):
    r['row_num'] = idx

print(f"Building updated Excel workbook with full Image URLs & Duplicate tracking...", flush=True)
...
ws_summary.append(["TAG Heuer Model", "Total Listings", "Unique Listings", "Duplicate Reposts", "% of Market", "Avg USD Price (WTS)"])
...
ws_summary.append(["Total TAG Heuer Listings Extracted", len(normalized_records)])
...
wb.save(excel_file)
print(f"Saved enhanced {excel_file} successfully!", flush=True)
