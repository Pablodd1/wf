...
BEGIN
  ...
  IF jsonb_array_length(p_hashes) > 1000 THEN
    RAISE EXCEPTION 'backfill_batch_limit';
  END IF;
  ...
  LOOP
    ...
    WHERE p.source_system = elem->>'source_system'
      AND p.source_database = elem->>'source_database'
      AND p.source_table = elem->>'source_table'
      AND p.source_id = elem->>'source_id'
      AND p.source_hash = elem->>'source_hash'
    FOR UPDATE;
    IF v_current IS NULL OR jsonb_typeof(elem->'expected_stored_proposal') IS DISTINCT FROM 'object'
      OR v_current IS DISTINCT FROM elem->'expected_stored_proposal' THEN
      RAISE EXCEPTION 'backfill_proposal_changed' USING ERRCODE = '40001';
    END IF;
