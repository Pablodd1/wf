...
function catalogReferenceKeys() {
  ...
  if (keys.length < 500) throw new Error('Checked-in Cartier catalog reference set is incomplete.');
  return keys;
}
...
WITH control AS MATERIALIZED (
  ...
  SELECT s.*,
    COALESCE(s.raw_message_text, '') ~* '(^|[^[:alnum:]])cartier([^[:alnum:]]|$)'
      AS raw_cartier_brand,
  ...
  SELECT jsonb_build_object(
    ...
    'cartier_source_rows', (SELECT count(*) FROM source),
    'raw_cartier_brand_rows', (SELECT count(*) FROM marked WHERE raw_cartier_brand),
    'raw_cartier_collection_rows', (SELECT count(*) FROM marked WHERE raw_cartier_collection),
    'catalog_cartier_reference_rows', (SELECT count(*) FROM marked WHERE catalog_cartier_reference),
...
async function main() {
  const output = path.resolve(process.argv[process.argv.indexOf('--output') + 1] || 'cartier-census.json');
  ...
  if (Number(report.evidence.cartier_source_rows || 0) < 1) throw new Error('No Cartier source rows found.');
  fs.mkdirSync(path.dirname(output), { recursive: true });
  fs.writeFileSync(output, `${JSON.stringify(report, null, 2)}\n`, 'utf8');
  console.log(JSON.stringify({
    cartier_source_rows: report.evidence.cartier_source_rows,
