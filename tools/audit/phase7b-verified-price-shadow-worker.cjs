...
const APPROVED_FX_SOURCES = new Set(['European Central Bank reference rates']);
...
function classifyObservation(row, catalog = catalogIndex()) {
  ...
  if (['USD', 'USDT'].includes(sourceCurrency)) {
    ...
    fxProvider = clean(row.conversion_source);
    fxRate = positive(row.conversion_rate);
    fxDate = clean(row.conversion_timestamp)?.slice(0, 10) || null;
    if (!fxProvider || !fxRate || !fxDate) {
      return finish(row, exact, 'FX_PROVENANCE_MISSING', 'FOREIGN_CURRENCY_HAS_NO_COMPLETE_DATED_FX_PROVENANCE', observation);
    }
    if (!APPROVED_FX_SOURCES.has(fxProvider)) {
      return finish(row, exact, 'FX_INVALID', 'FX_PROVIDER_IS_NOT_IN_THE_APPROVED_CONTRACT', observation);
    }
  ...
  if (Math.abs(verifiedUsd - currentUsd) > 1.01) {
    return finish(row, exact, 'SOURCE_PRICE_CONFLICT', 'CURRENT_NORMALIZED_USD_CONFLICTS_WITH_EXACT_SOURCE_EVIDENCE', observation,
      { sourceAmount, sourceCurrency, fxProvider, fxRate, fxDate });
  }

  return finish(row, exact, 'VERIFIED_IN_NEW_COHORT', null, observation,
    { sourceAmount, sourceCurrency, fxProvider, fxRate, fxDate, verifiedUsd, span });
}
...
async function run(options = {}) {
  ...
  for (const brand of BRANDS) {
    ...
    while (true) {
      ...
      const records = rows.map(row => classifyObservation(row, index));
      batchNumber += 1;
...
module.exports = { APPROVED_FX_SOURCES, BRANDS, CONTRACT, PARSER_VERSION, buildCatalog,
