...
function build(input, generatedAt = new Date().toISOString()) {
  ...
  const source = {
    ...
    query: {
      ...
      metric_definitions: {
        verified_observation: 'WTS observation with immutable lineage, exact source span, explicit amount and currency, exact canonical reference, and independently resolved official ECB historical FX when required.',
  ...
  const md = [
    ...
    '- Only parser-v5 exact source evidence with an explicit currency and independently resolved official ECB historical FX enters the verified cohort; stored conversion metadata is comparison evidence only.',
