-- Preserve WTB versus WTS intent in the on-demand contact action.
BEGIN;
...
BEGIN
 ...
 RETURN jsonb_build_object('contact_available',true,'dealer_id',d.id,'dealer_name',d.display_name,
  'dealer_profile_url','/reference-check/'||d.id::text,'dealer_rating',CASE WHEN d.review_count>0 AND d.rating>0 AND d.rating<=5 THEN d.rating ELSE NULL END,
  'dealer_review_count',d.review_count,'contact_phone',proof->>'source_identity','brand',v.brand,'reference',v.reference,'intent',v.intent);
END;
$$;
