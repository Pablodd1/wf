...
const PARSER_VERSION = 'expanded-evidence-v2-role-anchors';
...
function literalIntent(text) {
  ...
  for (const [intent, pattern] of [['WTB', buyPattern], ['WTS', sellPattern]]) {
    pattern.lastIndex = 0;
    for (const m of text.matchAll(pattern)) evidence.push({ intent, start: m.index, end: m.index + m[0].length });
  }
...
function makers(text) {
  ...
  return [...text.matchAll(makerPattern)].filter(m => {
    if (/^(?:pp|ap|rm|vc|lf|fpj)$/i.test(m[0]) && m[0] !== m[0].toUpperCase()) return false;
...
function prices(text, currency = null) {
  const claimed = extractPriceCandidates(text, currency ? { currency_context: currency } : {});
  ...
  for (const p of claimed) {
    ...
    const after = text.slice(position + quote.length, position + quote.length + 45);
    const adjustment = /^\s*[-+~–]\s*\d+(?:[.,]\d+)?\s*(?:%|[KM])?/i.exec(after);
    const supplemental = /\+\s*$/.test(linePrefix) || /\b(?:shipping\s*(?:fee|cost)?|deposit|additional|extra|service\s*fee)\s*[:=-]?\s*$/i.test(linePrefix) || /^\s*(?:to\s+make\s+complete|for\s+(?:shipping|box|papers))\b/i.test(after);
    ...
    found.push({ start: position, end: position + quote.length + (adjustment?.[0].length || 0), amount: numeric?.amount || null, currency: p.currency_original || null, numeric_evidence: numeric, rule: p.parser_rule, review_reason: adjustment || p.discount_percent != null ? 'PRICE_ADJUSTMENT_REQUIRES_REVIEW' : numeric ? p.review_reason : 'PRICE_NUMBER_FORMAT_REVIEW', role, explicit_sale_role: /\b(?:asking|sale\s+price|yours\s+for)\b/i.test(linePrefix), uses_currency_context: p.currency_evidence === 'section_currency' });
  }
...
function referenceClaims(text, brand, rawReference, priceFacts) {
  const result = [];
  const excludedAnchors = nonReferenceAnchors(text);
...
function buildExpandedCandidates(staged) {
  ...
  const lines = [];
  for (const match of view.text.matchAll(/[^\r\n\u2028\u2029]+/g)) {
    let start = match.index;
    for (const part of match[0].split('_x000D_')) {
      const leading = part.length - part.trimStart().length;
      const text = part.trim();
      if (text) lines.push({ text, start: start + leading, end: start + leading + text.length });
      start += part.length + '_x000D_'.length;
    }
  }
  const context = { brand: null, currency: null, intent: null, condition: null };
  const blocks = [];
  let active = null;
  ...
  for (const line of lines) {
    ...
    const condition = /^[\s*_#:-]*(?:(?:condition)\s*[:=-]?\s*)?(brand\s+new|unworn|used|pre[- ]owned|new)[\s*_#:-]*$/i.exec(line.text);
    ...
    if (!refs.length && (onlyMaker || curr || condition || onlyIntent || structured)) {
      ...
      if (onlyIntent || (structured && li.intent && !li.conflict)) context.intent = { value: li.intent, proof: makeSpan(line.start, line.end, 'SECTION_INTENT') };
    ...
    if (refs.length) {
      finish();
      active = { start: line.start, end: line.end, context: { ...context } };
...
module.exports = { CONTRACT, PARSER_VERSION, DEPENDENCY_HASHES, buildExpandedCandidates, verifyExpandedCandidate, parsingView, literalIntent };
