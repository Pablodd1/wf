...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Demand and WTS evidence must belong to the same committed publication.
    let demandSnapshotId;
    {
      const { data: openedDemandSnapshot, error: demandSnapshotError } = await supabase.rpc("get_price_research_demand_snapshot", { p_snapshot_id: snapshotId });
      if (demandSnapshotError) throw mapSnapshotRpcError(demandSnapshotError) || demandSnapshotError;
      ...
      if (parsedDemandCursor && parsedDemandCursor.snapshot !== demandSnapshotId) {
        return res.status(400).json({ success: false, error: "Demand cursor belongs to a different publication. Restart both evidence lanes." });
      }
    }
    ...
    const { data: facetRows, error: facetErr } = await supabase.rpc("get_price_research_snapshot_facets", {
      p_snapshot_id: snapshotId,
      p_brand: brandQuery,
      p_reference: referenceQuery,
      p_model: modelQuery,
    ...
    const { data: breakdownRows, error: breakdownErr } = await supabase.rpc("get_price_research_snapshot_breakdown", {
      p_snapshot_id: demandSnapshotId,
      p_brand: brandQuery,
      p_reference: referenceQuery,
      p_model: modelQuery,
    ...
    if (!cohortBreakdown) throw new Error('Snapshot breakdown unavailable');
    for (const key of ['source_observations', 'wts_count', 'wtb_count', 'unique_qualified_offers',
      'included_count', 'excluded_duplicates', 'excluded_ambiguous_currency', 'excluded_unsupported_fx',
      'excluded_implausible', 'excluded_iqr_outliers', 'excluded_not_wts', 'excluded_ineligible_flag',
      'retained_audit_evidence_count']) {
      const raw = cohortBreakdown[key];
      if (!/^(0|[1-9][0-9]*)$/.test(String(raw)) || !Number.isSafeInteger(Number(raw))) {
        throw new Error('Invalid snapshot breakdown');
      }
    ...
    if (cohortBreakdown.source_observations !== cohortBreakdown.included_count + excludedTotal
      || cohortBreakdown.retained_audit_evidence_count !== excludedTotal) throw new Error('Unreconciled snapshot breakdown');
    ...
    return res.status(200).json({
      ...
      totalListings: cohortBreakdown.source_observations,
      reference_listing_count: cohortBreakdown.source_observations,
      wts_count: wtsTotalCount,
      wtb_count: wtbTotalCount,
      qualified_count: cohortBreakdown.included_count,
      retained_evidence_count: cohortBreakdown.retained_audit_evidence_count,
      excludedEvidenceCount: excludedTotal,
      outliersRemoved: cohortBreakdown.excluded_iqr_outliers,
      excluded_counts: {
        not_wts: cohortBreakdown.excluded_not_wts,
        unresolved_currency: cohortBreakdown.excluded_ambiguous_currency + cohortBreakdown.excluded_unsupported_fx,
        ineligible_flag: cohortBreakdown.excluded_ineligible_flag,
        duplicate_repost: cohortBreakdown.excluded_duplicates,
    ...
    return res.status(status).json({ success: false, error: status === 503 ? "Service temporarily unavailable" : "Unable to load price research" });
  }
};
